...
describe('TTS comic consensus adapter', () => {
  test('loads in-place comic audio outputs and selected render cost', async () => {
    ...
    roots.push(runDir)
    const artifactDir = 'audio/providers/inworld-realtime-tts-2'
    ...
    await writeFile(join(runDir, 'manifest.json'), JSON.stringify({
      ...
      items: [{
        status: 'full', metadata: { tts: [{ ttsService: 'inworld', ttsModel: 'realtime-tts-2', processingTime: 1234, audioFileName: 'audio/final/inworld_realtime-tts-2.wav', audioFileSize: 456, chunkCount: 51 }] },
        providers: [{ service: 'inworld', model: 'realtime-tts-2', artifactDir, status: 'succeeded', attempts: 1, options: {}, metadata: { comicAudio: projection }, result: { comicAudio: projection } }],
      }],
    }))

    const record = loadTtsManifestRecord(runDir)
    expect(record.metadata.tts).toEqual([expect.objectContaining({ ttsService: 'inworld', ttsModel: 'realtime-tts-2', chunkCount: 51 })])
    expect(record.metadata.cost?.actual?.steps).toEqual([{ provider: 'inworld', model: 'realtime-tts-2', cost: 12.3 }])
  })
})
