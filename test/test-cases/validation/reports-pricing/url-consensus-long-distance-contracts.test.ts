...
describe('URL consensus long-distance contracts', () => {
  test('keeps long identical, edited, inserted, and truncated sequences deterministic', () => {
    const reference = Array.from({ length: 10_001 }, (_, index) => `token-${index}`)
    const substitutions = [...reference]
    for (let index = 100; index < substitutions.length; index += 200) {
      substitutions[index] = `changed-${index}`
    }
    const inserted = [...reference.slice(0, 5_000), 'inserted', ...reference.slice(5_000)]
    const truncated = reference.slice(0, 2_500)

    expect(levenshteinBreakdown(reference, reference).distance).toBe(0)
    expect(levenshteinBreakdown(reference, substitutions).distance).toBeGreaterThanOrEqual(45)
    expect(levenshteinBreakdown(reference, substitutions).distance).toBeLessThanOrEqual(55)
    expect(levenshteinBreakdown(reference, inserted).distance).toBe(1)
    expect(levenshteinBreakdown(reference, truncated).distance).toBe(7_501)
  })
  ...
  test('scales separated long-sequence insertions and deletions back to edit units', () => {
    const reference = Array.from({ length: 10_001 }, (_, index) => String.fromCodePoint(0x1000 + index))
    const inserted = [
      ...reference.slice(0, 2_500),
      String.fromCodePoint(0x9000),
      ...reference.slice(2_500, 5_000),
      String.fromCodePoint(0x9001),
      ...reference.slice(5_000, 7_500),
      String.fromCodePoint(0x9002),
      ...reference.slice(7_500)
    ]
    const deletedIndexes = new Set([2_500, 5_000, 7_500])
  ...
  test('uses extended exact alignment before accepting a coarse token-anchor estimate', () => {
    const uniquePrefix = Array.from({ length: 2_001 }, (_value, index) => `unique-${index}`)
    const repeatedTail = [
  ...
  test('keeps long reordered-block anchor estimates symmetric', () => {
    const blocks = Array.from({ length: 10 }, (_value, blockIndex) =>
      Array.from({ length: 1_001 }, (_blockValue, tokenIndex) =>
    ...
    expect(forward).toBe(6_006)
    expect(reverse).toBe(forward)
  })

  test('scores distributed patience-anchor gaps by their changed content', () => {
    const pairs = Array.from({ length: 5_001 }, (_value, index) => [
      `unique-${index}`,
      'common'
    ])
    const reference = pairs.flat()
    const withLateCommonTokensDeleted = pairs.flatMap((pair, index) =>
      index >= 2_952 ? [pair[0]!] : pair
    )
    const candidate = [
      ...withLateCommonTokensDeleted.slice(0, 2),
      ...Array.from({ length: 2_049 }, (_value, index) => `inserted-${index}`),
      ...withLateCommonTokensDeleted.slice(2)
    ]

    expect(reference).toHaveLength(10_002)
    expect(candidate).toHaveLength(10_002)
    expect(levenshteinBreakdown(reference, candidate).distance).toBe(4_098)
    expect(levenshteinBreakdown(candidate, reference).distance).toBe(4_098)
  })

  test('preserves section order when long sequences contain repeated elements', () => {
    const reference = [...new Array<string>(5_001).fill('a'), ...new Array<string>(5_001).fill('b')]
    const reordered = [...new Array<string>(5_001).fill('b'), ...new Array<string>(5_001).fill('a')]

    expect(levenshteinBreakdown(reference, reordered).distance).toBe(10_002)
    expect(levenshteinBreakdown(reordered, reference).distance).toBe(10_002)
  })
  ...
  test('keeps repeated-sequence substitutions above the bounded exact limit', () => {
    ...
    expect(changedIndexes).toHaveLength(2_049)
    expect(levenshteinBreakdown(reference, substituted).distance).toBe(2_049)
    expect(levenshteinBreakdown(substituted, reference).distance).toBe(2_049)
  })

  test('preserves alignment across multiple large inserted blocks', () => {
    const reference = Array.from({ length: 10_001 }, (_value, index) => String(index % 10))
    const inserted = [
      ...reference.slice(0, 3_000),
      ...new Array<string>(1_001).fill('x'),
      ...reference.slice(3_000, 7_000),
      ...new Array<string>(1_001).fill('y'),
      ...reference.slice(7_000)
    ]
    const insertedAndSubstituted = [...inserted]
    insertedAndSubstituted[6_000] = 'z'

    expect(levenshteinBreakdown(reference, inserted).distance).toBe(2_002)
    expect(levenshteinBreakdown(inserted, reference).distance).toBe(2_002)
    expect(levenshteinBreakdown(reference, insertedAndSubstituted).distance).toBe(2_003)
  })
