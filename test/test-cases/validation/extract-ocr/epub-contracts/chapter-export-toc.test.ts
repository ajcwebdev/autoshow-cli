...
test('EPUB chapter export groups spine fragments by TOC starts', async () => {
  ...
  expect(output.exportPlan?.files.map((file) => file.relativePath)).toEqual([
    'chapters/01-002-chapter-one.txt',
    'chapters/02-005-chapter-two.txt'
  ])
...
test('EPUB chapter export splits multiple TOC fragments within one spine file', async () => {
  ...
  expect(files.map((file) => file.relativePath)).toEqual([
    'chapters/01-001-table-of-contents.txt',
    'chapters/02-001-introduction.txt',
    'chapters/03-001-chapter-one.txt',
    'chapters/04-001-chapter-two.txt',
    'chapters/05-002-chapter-three.txt'
  ])
