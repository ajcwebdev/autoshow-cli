...
import { selectBatchItems } from '~/cli/commands/sources/metadata/metadata-batch/metadata-batch-select'
import { resolveProcessTargetDoubleDash } from '~/cli/commands/sources/download/download-targets/handle-process-target'
import { buildDownloadMediaOptions } from '~/cli/commands/sources/download/download-targets/single/media-runner'
...
`)
  ...
  await writeFile(ffprobePath, '#!/bin/sh\nif [ "$1" = "-v" ]; then echo "{\"streams\":[{\"index\":0,\"codec_type\":\"audio\",\"codec_name\":\"mp3\",\"channels\":1,\"bit_rate\":\"64000\"}],\"format\":{\"format_name\":\"mp3\",\"duration\":\"1\",\"bit_rate\":\"64000\"}}"; else echo 1; fi\nexit 0\n')
...
describe('yt-dlp passthrough execution contracts', () => {
  test('raw mode streams yt-dlp output and reports child failures through the CLI error contract', async () => {
    ...
    const env = fakeToolEnv(fakeBinDir)
