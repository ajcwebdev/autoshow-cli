...
export const registerMediaCommandHelpCases = (): void => {
  ...
  test.concurrent('image and video help expose generic provider selection plus their own option groups', async () => {
    ...
    expect(videoResult.stdout).not.toContain('--mode edit')
    expect(videoResult.stdout).not.toContain('--mode extend')
    expect(videoResult.stdout).not.toContain(' extend')
    expect(helpText(videoResult.stdout)).not.toMatch(/\bedit\b/)
    expect(helpText(videoResult.stdout)).not.toMatch(/\bextend\b/)
