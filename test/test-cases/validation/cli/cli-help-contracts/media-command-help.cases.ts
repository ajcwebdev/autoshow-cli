...
export const registerMediaCommandHelpCases = (): void => {
  test.concurrent('tts help exposes hosted TTS provider flags', async () => {
    ...
    expect(getFlagGroupSection(result.stdout, 'Concurrency')).toContain('--step-concurrency')
    ...
    expect(result.stdout).toContain('--step-concurrency')
    expect(helpText(result.stdout)).toContain('tts-chunk:')
    expect(helpText(result.stdout)).toContain('50 for a Grok-only selection')
    ...
    expect(result.stdout).not.toContain('--minimax-tts-volume')
    expect(result.stdout).not.toContain('--minimax-tts-pitch')
    expect(result.stdout).not.toContain('--minimax-tts-emotion')
    expect(result.stdout).not.toContain('--minimax-tts-english-normalization')
    expect(result.stdout).not.toContain('--minimax-tts-pronunciation')
    ...
    expect(result.stdout).not.toContain('--elevenlabs-tts-stability')
    expect(result.stdout).not.toContain('--elevenlabs-tts-similarity-boost')
    expect(result.stdout).not.toContain('--elevenlabs-tts-style')
    expect(result.stdout).not.toContain('--elevenlabs-tts-use-speaker-boost')
    expect(result.stdout).not.toContain('--elevenlabs-tts-speed')
    expect(result.stdout).not.toContain('--elevenlabs-tts-seed')
    expect(result.stdout).not.toContain('--elevenlabs-tts-text-normalization')
    expect(result.stdout).not.toContain('--elevenlabs-tts-pronunciation-dictionary-locator')
    expect(result.stdout).not.toContain('--elevenlabs-tts-optimize-streaming-latency')
    // Provider-general spellings backed by the same ElevenLabs controls.
    expect(result.stdout).toContain('--tts-stability')
    expect(result.stdout).toContain('--tts-similarity')
    expect(result.stdout).toContain('--tts-style')
  ...
  test.concurrent('image and video help expose generic provider selection plus their own option groups', async () => {
    ...
    expect(imageResult.stdout).toContain('gpt-image-2.5-flare')
    expect(imageResult.stdout).toContain('gpt-image-2.5-sunburst')
    expect(imageResult.stdout).toContain('xhigh')
    expect(imageResult.stdout).toContain('replicate')
    expect(imageResult.stdout).toContain('alibaba/qwen-image-3')
    expect(imageResult.stdout).not.toContain('bfl')
    expect(imageResult.stdout).not.toContain('--search-grounding')
    expect(videoResult.stdout).toContain('ltx')
    expect(videoResult.stdout).toContain('ltx-2-5-fast')
    expect(videoResult.stdout).toContain('LTX 2.5')
    expect(videoResult.stdout).not.toContain('ltx-2-3-fast')
    expect(videoResult.stdout).not.toContain('ltx-2-3-pro')
    expect(videoResult.stdout).not.toContain('grok-imagine-video,')
    expect(videoResult.stdout).not.toContain('kwaivgi/kling-v3-video')
    expect(videoResult.stdout).not.toContain('kwaivgi/kling-v3-omni-video')
    expect(videoResult.stdout).not.toContain('bytedance/seedance-2.0')
    expect(videoResult.stdout).not.toContain('fal-ai/pixverse/c1')
    expect(videoResult.stdout).toContain('gemini-omni-1.1-flash')
    expect(videoResult.stdout).not.toContain('veo-3.1-lite-generate-preview')
    expect(videoResult.stdout).not.toContain('veo-3.1-generate-preview')
    expect(videoResult.stdout).not.toContain('veo-3.1-fast-generate-preview')
    expect(videoResult.stdout).toContain('edit')
    expect(videoResult.stdout).toContain('extend')
    expect(videoResult.stdout).toContain('--previous-interaction-id')
  ...
  test.concurrent('Luma, ratio, and music duration descriptions match supported behavior', async () => {
    ...
    expect(helpText(music.stdout)).toContain('Gemini Lyria uses the requested duration as a prompt hint')
    expect(helpText(music.stdout)).not.toContain('Gemini Lyria Pro uses the requested duration')
    expect(helpText(image.stdout)).toContain('(default: 1)')
  })

  test.concurrent('video help omits retired Kling multi-prompt', async () => {
    const video = await loadHelp(['video', '--help'])
    const write = await loadHelp(['write', '--help'])

    expect(helpText(video.stdout)).not.toContain('--replicate-video-multi-prompt')
