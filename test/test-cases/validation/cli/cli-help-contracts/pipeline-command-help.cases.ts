...
export const registerPipelineCommandHelpCases = (): void => {
  ...
  test.concurrent('extract help exposes shared batch and all-provider flags', async () => {
    ...
    expect(transcriptionSection).toContain('--stt-language')
    expect(transcriptionSection).toContain('[default: ["scrapecreators=en"]]')
    ...
    expect(documentSection).not.toContain('--ocr-concurrency')
    expect(documentSection).toContain('--ocr-provider-mode')
    expect(documentSection).toContain('fanout|pool')
    // The page-level OCR cap moved into the --step-concurrency ocr-page scope.
    expect(helpText(result.stdout)).toContain('local OCR uses 10')
    expect(helpText(result.stdout)).toContain('hosted OCR chooses per run')
    expect(documentSection).toContain('--chapters')
    expect(documentSection).toContain('--no-chapters')
    // The URL route now falls back to the shared --provider-concurrency lane.
    expect(articleSection).not.toContain('--url-provider-concurrency')
    ...
    expect(result.stdout).toContain('--stt-language')
    ...
    expect(result.stdout).toContain('stt-preflight:')
  ...
  test.concurrent('write and setup help expose shared selectors and concurrency flags', async () => {
    ...
    expect(writeResult.stdout).toContain('grok=grok-4.6')
    ...
    expect(helpText(setupResult.stdout)).toContain('gemini|openai|grok|replicate')
    expect(setupResult.stdout).toContain('--step-concurrency')
    expect(helpText(writeResult.stdout)).not.toContain('50 for a Grok-only selection')
    expect(helpText(setupResult.stdout)).toContain('50 for a Grok-only selection')
    expect(helpText(setupResult.stdout)).toContain('ocr-page:')
  ...
  test.concurrent('resume help exposes unified multi-target provider selector', async () => {
    ...
    expect(result.stdout).toContain('--provider-concurrency')
    expect(result.stdout).toContain('--local-concurrency')
    expect(result.stdout).toContain('ocr-page:')
    ...
    expect(result.stdout).not.toContain('bfl')
