...
const ACTIVE_TTS_PROVIDERS = ['elevenlabs', 'minimax', 'grok', 'mistral', 'openai', 'speechify', 'hume', 'cartesia', 'inworld'] as const
...
describe('option resolution contracts', () => {
  test('--all-tts expands exactly the active provider registry', () => {
    ...
    for (const retired of ['groq', 'gemini', 'deepgram', 'replicate', 'fal', 'fish', 'deepinfra']) expect(selections).not.toHaveProperty(retired)
  })
