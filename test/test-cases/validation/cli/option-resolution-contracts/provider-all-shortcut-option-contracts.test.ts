...
describe('option resolution contracts', () => {
  ...
  test('--all-stt/--all-ocr expand hosted providers and local shortcuts expand local engines', () => {
      ...
      expect(ocrOpts.mistralOcrModels).toEqual(['mistral-ocr-2512', 'mistral-ocr-4-0', 'mistral-ocr-4-1'])
      expect(ocrOpts.openaiOcrModels).toEqual(['gpt-6-astra', 'gpt-5.6-sol', 'gpt-5.6-terra', 'gpt-5.6-luna', 'gpt-5.5', 'gpt-5.4-mini', 'gpt-5.4-nano'])
      ...
      expect(ocrOpts.anthropicOcrModels).toEqual(['claude-fable-5-1', 'claude-fable-5', 'claude-opus-4-8', 'claude-sonnet-5', 'claude-sonnet-4-6', 'claude-haiku-4-5', 'claude-opus-5'])
      expect(ocrOpts.anthropicOcrModels).not.toContain('claude-mythos-5')
      expect(ocrOpts.geminiOcrModels).toEqual(['gemini-3.1-pro-preview', 'gemini-3.8-flash', 'gemini-3.7-flash', 'gemini-3.5-flash', 'gemini-3.6-flash', 'gemini-3.5-flash-lite'])
      expect(ocrOpts.geminiOcrModels).not.toContain('gemini-3.1-flash-lite')
      expect(ocrOpts.deepinfraOcrModels).toEqual(['google/gemma-3-27b-it', 'google/gemma-4-31B-it', 'google/gemma-4-26B-A4B-it', 'meta-llama/Llama-4-Scout-17B-16E-Instruct', 'mistralai/Mistral-Small-3.2-24B-Instruct-2506', 'Qwen/Qwen3-VL-235B-A22B-Instruct', 'Qwen/Qwen3-VL-30B-A3B-Instruct'])
