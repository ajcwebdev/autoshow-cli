...
describe('option resolution contracts', () => {
  ...
  test('--all-stt/--all-ocr expand hosted providers and local shortcuts expand local engines', () => {
      ...
      expect(ocrOpts.mistralOcrModels).toEqual(['mistral-ocr-4-0', 'mistral-ocr-4-1'])
      expect(ocrOpts.openaiOcrModels).toEqual(['gpt-6-astra', 'gpt-5.6-sol', 'gpt-5.6-terra', 'gpt-5.6-luna'])
      expect(ocrOpts.openaiOcrModels).not.toContain('gpt-5.6')
      expect(ocrOpts.grokOcrModels).toEqual(['grok-4.5', 'grok-4.6'])
      expect(ocrOpts.kimiOcrModels).toEqual(['kimi-k2.6', 'kimi-k3'])
      expect(ocrOpts.anthropicOcrModels).toEqual(['claude-fable-5-1', 'claude-fable-5', 'claude-sonnet-5', 'claude-opus-5'])
      expect(ocrOpts.anthropicOcrModels).not.toContain('claude-mythos-5')
      expect(ocrOpts.geminiOcrModels).toEqual(['gemini-3.8-flash', 'gemini-3.7-flash', 'gemini-3.5-flash', 'gemini-3.6-flash', 'gemini-3.5-flash-lite'])
      expect(ocrOpts.geminiOcrModels).not.toContain('gemini-3.1-flash-lite')
      expect(ocrOpts.deepinfraOcrModels).toEqual(['google/gemma-4-31B-it'])
