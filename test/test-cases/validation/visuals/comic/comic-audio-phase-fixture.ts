...
import { createApprovedVoiceSnapshotEntry, createComicSourceIdentity, createStructuredScriptArtifactRef, computeSceneRunIdentity, validateVoiceReferenceManifest } from '~/cli/commands/visuals/comic/comic-utils/comic-audio-contracts'
...
export const buildComicAudioPhase2SnapshotEntry = (
  subjectKey: string,
  resourceId: string,
  provider: 'elevenlabs' | 'inworld' | 'openai' = 'elevenlabs',
  providerModel = provider === 'elevenlabs'
    ? 'eleven_v3'
    : provider === 'inworld'
      ? 'realtime-tts-2'
