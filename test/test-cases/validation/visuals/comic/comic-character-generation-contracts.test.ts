...
import { characterSketchCommand } from '~/cli/commands/visuals/comic/comic-commands/character-sketch/character-sketch-command'
import { checksumFile,getCharacterSketchManifestPath,requireCurrentCharacterSketch } from '~/cli/commands/visuals/comic/comic-commands/process-scenes/character-utils'
...
describe('comic character handling flat-reference contracts', () => {
  ...
  test('reference-sketch character parsing rejects --image and enforces one model', () => {
    ...
    expect(() => parseReferenceSketchArgs(['--character', 'hero', '--provider', 'openai=gpt-image-2', '--provider', 'grok=grok-imagine-image-2.0'])).toThrow(/exactly one/)
