...
import { getLocationReferencePath, getLocationSketchManifestPath, readLocationReferenceCatalog } from '~/cli/commands/process-steps/step-8-comic/comic-utils/location-reference'
...
describe('canonical location reference registration', () => {
  test('derives a missing location catalog style reference from the configured character catalog', async () => {
    ...
    await Bun.write(join(characters, 'characters-reference.json'), JSON.stringify({
      schemaVersion: 3,
      characters: [{ key: 'artist', name: 'Artist', aliases: [], image: 'artist.webp', outlineSheet: 'artist.webp', description: 'Project-defined visual style reference.' }],
      groupAliases: [],
    }))
    configureCharactersRoot(characters)

    expect(await readLocationReferenceCatalog()).toEqual({
      schemaVersion: 1,
      styleImage: '../characters/artist.webp',
      locations: [],
    })
  })
  ...
  test('aggregates matching scripts once, preserves provenance, chains views, and registers only after QA success', async () => {
    ...
    expect(catalog.locations[0].specification).not.toContain('mechanic')
