...
import * as recoveryFacade from '~/cli/commands/audio/tts/script-to-audio/attempt-recovery'
import {
  resolveRetainedPath,
  validateRecoveryProjections,
  reconcileSlotCosts,
  buildPureCurrentTtsRenderPlan,
} from '~/cli/commands/audio/tts/script-to-audio/current-render-attempt'
import type {
  CompatibleSignature,
  CompletedSignature,
...
describe('TTS recovery helper modules', () => {
  ...
  it('keeps facade orchestration thin and recovery publication explicitly ordered', () => {
    const facadePath = resolve(
      process.cwd(),
      'src/cli/commands/audio/tts/script-to-audio/attempt-recovery.ts'
    )
    ...
    const finalizationSource = readFileSync(resolve(
      process.cwd(),
      'src/cli/commands/audio/tts/script-to-audio/recovery-finalization.ts'
    ), 'utf8')
