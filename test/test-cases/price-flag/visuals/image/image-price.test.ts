...
test('--price allows multiple image providers and reports each image step', async () => {
  const result = await runCommand(
    ['src/cli/create-cli.ts', 'image', 'a sunset', '--provider', 'openai=gpt-image-2', '--provider', 'grok=grok-imagine-image-2.0', '--price', '--json'],
  )
  ...
  expect(output).toContain('generated-image-grok-grok-imagine-image-2.0.jpg')
})
...
test('image --out in price mode reports explicit output directory without creating it', async () => {
  ...
  try {
    ...
    const result = await runCommand(
      ['src/cli/create-cli.ts', 'image', 'a sunset over a lake', '--provider', 'openai=gpt-image-2', '--output-dir', outputDir, '--price', '--json'],
    )
    const output = `${result.stdout}\n${result.stderr}`
