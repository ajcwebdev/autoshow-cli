...
import { statPath as stat } from '../../src/utils/bun-file-io'
import { join, relative, resolve } from 'node:path'
import type { CliOutcome, LocalExecutionAdapter } from '../scenarios/local-cli-contracts'
import { artifactExists } from '../scenarios/local-cli-contracts'
...
export async function discoverOutputDir(root: string): Promise<string | null> {
  ...
  requireCondition(candidates.length <= 1, `Ambiguous output runs under ${root}`)
...
export class DockerScenarioAdapter implements LocalExecutionAdapter {
  ...
  async execute(args: string[]): Promise<CliOutcome> {
    expect(args, 'Scenario changed its authorized command').toStrictEqual(this.scenario.args)
    const network = this.scenario.network === 'public' ? 'bridge' : this.scenario.network === 'fixture' ? this.engine.fixtureNetwork : 'none'
    requireCondition(network, 'Fixture network is unavailable')
    ...
    requireCondition(!result.timedOut, `${this.scenario.id} timed out`)
  ...
  async probe(path: string): Promise<{ codec_name: string; width: number; height: number }> {
    ...
    requireCondition(!suffix.startsWith('..'), 'Probe must inspect an acceptance output')
    ...
    requireCondition(data.streams[0], 'No video stream')
...
export async function runDockerAcceptance(options: DockerOptions, engine = new DockerEngine(options)): Promise<number> {
  ...
  try {
    ...
    requireCondition(identity.cliVersion, 'Missing CLI version')
