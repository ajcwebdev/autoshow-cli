...
- [ ] For an archive root, run `scripts/run.ts <category> compact-archive`, then `build-combined-report` when the category supports it; that rebuilds `docs/benchmarks/combined-comparison-dashboard.html` automatically when all five combined-report roots are present.
...
`build-combined-report` (STT, OCR, and URL) aggregates every per-run report JSON under a root directory into generated `combined-comparison-report.{json,md}` artifacts, matching providers by `providerKey` and ranking price, speed, and quality within the same category groups. STT also promotes observed realtime throughput, computed as total covered audio duration divided by total covered processing time, into its JSON, Markdown, and the dashboard. URL reads committed `provider-comparison-report.json` files and optional sibling `manifest.json` metadata; it uses source `rankingSurfaces.*.automatedQuality` values instead of recomputing quality. `build-combined-dashboard` writes the repository-level `docs/benchmarks/combined-comparison-dashboard.html` shell together with its generated siblings, all named by relative path: the data `combined-comparison-dashboard.json`, the stylesheet `combined-comparison-dashboard.css`, and the script `combined-comparison-dashboard.js`. The script fetches the JSON and renders a zero-dependency page with tabs for `ocr`, `stt-local`, `stt-with-speakers`, `stt-without-speakers`, `url`, and `tts` in the browser, so the page needs JavaScript and an HTTP origin. The first five consolidate combined-report data; TTS reads selected manifest artifacts directly and separates narration rankings from emotion/delivery and speed/pause cases within one TTS tab. The controls benchmarks have independent directories and revision selection. Each group's metric table can be sorted by quality, cost, or speed. Each group's sort control also offers `Custom`, three sliders sharing one 100% budget that rescore that group against the reader's own quality/speed/cost trade-off; it is a browser-side viewing aid and writes nothing to any artifact. `build-combined-report` re-runs it automatically when its parent directory holds all five combined-report roots; `benchmarks_root` defaults to `docs/benchmarks`. Combined reports do not emit weighted composites, overall scores, or model tiers; the dashboard's reader-chosen weighting is computed in the browser and never written to the JSON or Markdown. Groups are never ranked against each other. See `references/stt.md`, `references/ocr.md`, `references/url.md`, and `references/tts.md` for details.
...
10. For OCR, STT, URL, and TTS combined HTML dashboards, confirm each group's metric table can be sorted by quality, cost, or speed without JavaScript or network requests.
