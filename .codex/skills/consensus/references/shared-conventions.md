...
Every packet, report, and compacted artifact is written through `scripts/shared/portable_paths.ts`, which rewrites absolute paths inside the repository checkout (including `file://` URLs) to repository-relative paths, so `Run directory`, `Root directory`, `runDir`, and `rootDir` never embed a machine-specific location. Paths outside the checkout are left unchanged.
