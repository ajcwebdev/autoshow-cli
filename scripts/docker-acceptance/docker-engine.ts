...
export function mapArtifactPath(path: string, mounts: MountMapping[], relativeTo = '/results'): string {
  ...
  assert(mount, `Artifact path is outside the acceptance mounts: ${path}`)
  const mapped = resolve(mount.host, posix.relative(mount.container, absolute))
  const suffix = relative(mount.host, mapped)
  assert(!suffix.startsWith('..') && !isAbsolute(suffix), `Artifact escaped mount: ${path}`)
  return mapped
}
...
export function containerArguments(identity: ImageIdentity, name: string, runId: string, mounts: MountMapping[], network: string, args: string[], entrypoint?: string): string[] {
  assert(identity.reference.startsWith(`${IMAGE_REPOSITORY}@sha256:`), 'An immutable GHCR reference is required')
  ...
  for (const mount of mounts) {
    ...
    assert(!['/app', '/app/src', '/app/node_modules'].includes(mount.container), 'Cannot replace image source')
...
export class DockerEngine {
  ...
  requireSuccess(result: ProcessOutcome, purpose: string): ProcessOutcome {
    assert(!result.timedOut, `${purpose} timed out; see logs in ${this.options.output}`)
    assert.equal(result.exitCode, 0, `${purpose} failed:\n${result.stderr.slice(-6000)}`)
    return result
  }
  ...
  async resolveImage(): Promise<ImageIdentity> {
    ...
    assert(!this.options.platform || this.options.platform === platform, `--platform ${this.options.platform} is not native to this ${platform} Docker daemon`)
    ...
    assert.equal(digests.length, 1, 'Pull did not report one immutable digest')
    ...
    assert.equal(`${image.Os}/${image.Architecture}`, platform, 'Pulled architecture differs from native daemon')
    assert(image.Config.User && !['0', 'root'].includes(image.Config.User.split(':')[0]!), 'Acceptance requires the image normal non-root user')
    assert(image.Config.Entrypoint?.length, 'Image must provide a CLI entrypoint')
    for (const value of image.Config.Env ?? []) {
      const [key, ...rest] = value.split('=')
      assert(!/(API_KEY|TOKEN|SECRET|PASSWORD|CREDENTIAL)/i.test(key!) || !rest.join('='), `Image contains credential setting: ${key}`)
    }
    ...
    if (this.options.expectedDigest) assert.equal(digest, this.options.expectedDigest, 'latest changed since publication')
    if (this.options.expectedRevision) assert.equal(this.identity.revision, this.options.expectedRevision, 'Image revision differs from published commit')
  ...
  async repairCacheOwnership(): Promise<void> {
    ...
    assert.equal(mounts.length, 2, 'Cache ownership repair requires only the two asset mounts')
  ...
  async cleanup(): Promise<void> {
    ...
    assert.equal(this.owned.size, 0, 'Could not remove suite-owned containers; inspect retained logs')
    assert(!this.fixtureNetwork, 'Could not remove suite-owned fixture network; inspect retained logs')
  }
}
