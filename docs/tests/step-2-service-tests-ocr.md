...
- `test/test-cases/e2e/service/step-2-ocr-e2e/ocr-services/ocr-supadata.test.ts` covers remote article extraction with `--url-provider supadata`, writes `extraction.txt`, and records `step2.extractionMethod: "html+supadata"` in `run.json`.
