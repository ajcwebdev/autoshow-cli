...
1. [System Overview & CLI Surface](diagrams/01-system-overview-cli.md) - High-level architecture, the 14-command native CLI surface, native dispatch and command routing, provider selectors, and global runtime flags such as config, cookies, and model path
...
5. [Types, Metadata & Output Layout](diagrams/05-types-and-output.md) - Output schema v2, run metadata, output directory structure, provider result artifacts, and the type system by pipeline step
