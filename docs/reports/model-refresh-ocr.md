...
This report is one of eight per-modality records split on 2026-08-19 from the former consolidated 2026 hosted-model refresh ledger (retired as an ADR; the remaining ADRs were renumbered to close the gap). Sibling reports: [STT](model-refresh-stt.md), [URL scraping](model-refresh-url.md), [LLMs](model-refresh-write.md), [TTS](model-refresh-tts.md), [Music](model-refresh-music.md), [Image](model-refresh-image.md), [Video](model-refresh-video.md).
...
- OCR provider adapters: `src/cli/commands/text/ocr/`
...
Gemini 3.8 Flash is available as `gemini-3.8-flash` for writing/OCR (`gemini`), with existing selectors and defaults preserved. Writing and OCR support low/medium/high reasoning; minimal and disabled are rejected. Requests omit legacy sampling controls; the transport rejects incompatible 3.8 settings before dispatch. STT no longer uses Flash; see the [STT refresh](model-refresh-stt.md#2026-09-14-gemini-35-transcribe-replacement) for `gemini-3.5-transcribe`.
