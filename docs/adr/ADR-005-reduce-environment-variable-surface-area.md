...
- **Reason kept:** Credentials must enter the process; env is the standard channel with no better alternative

**Var(s) 2: Runner-to-child IPC vars (`AUTOSHOW_TEST_OUTPUT_DIR`, `…_TEST_ARTIFACTS_DIR`, `…_TEST_COMMAND_LOG`, `…_TEST_METRICS_LOG`, `…_TEST_PRESERVE_ARTIFACTS`, `…_TEST_BUDGET_SKIP_KEYS`, `…_TEST_BUDGET_EVALUATED_KEYS`, `…_PROCESS_LOCK_DIR`, `LOCK_ROOT`)**

- **Var(s):** Runner-to-child IPC vars (`AUTOSHOW_TEST_OUTPUT_DIR`, `…_TEST_ARTIFACTS_DIR`, `…_TEST_COMMAND_LOG`, `…_TEST_METRICS_LOG`, `…_TEST_PRESERVE_ARTIFACTS`, `…_TEST_BUDGET_SKIP_KEYS`, `…_TEST_BUDGET_EVALUATED_KEYS`, `…_PROCESS_LOCK_DIR`, `LOCK_ROOT`)
- **Reason kept:** Env is the correct mechanism for passing state into spawned child processes
...
- **Reason kept:** Consolidated binary override used in test helper runner IPC
