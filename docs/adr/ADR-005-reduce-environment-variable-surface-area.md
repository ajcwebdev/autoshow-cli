...
| Pass 4 follow-up: decide how many `baseUrl` params the step-3 write providers should keep | Provider maintainers | Closed 2026-08-07 as **none** — see the note below |

> Closure (2026-08-07): pass 4 left a run-level `baseUrl` 4th parameter on four write providers (`runAnthropicModel`, `runGrokModel`, `runCerebrasModel`, `runTogetherModel`); the open question was whether any should survive. The answer is none. `LLMTarget['run']` is typed 3-ary and `run-llm.ts` invokes it with three arguments, so production could never reach the parameter, and only two contract tests passed it. Endpoint injection keeps the one home it already had — the `baseURL` on the config object accepted by `runAnthropicCompatibleModel`/`runOpenAICompatibleChatModel`, which the OpenAI-compatible suites already exercise directly. The four parameters are removed, along with the now-constant `resolveCerebrasBaseUrl`/`resolveTogetherBaseUrl` indirection and the equally dead `baseUrl` parameters on `getAnthropicClientConfig`/`getGrokClientConfig`; each provider now reads its `base-urls.ts` constant directly. The two tests moved to default-host assertions. Env-var overrides stay deleted; nothing in this closure reopens them.
