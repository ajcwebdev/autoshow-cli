...
1. **The contracts live in the suite.** `test/test-cases/validation/runtime-contracts/output-vocabulary-contracts.test.ts` scans every `.ts` file under `src/` and `test/` and fails with the offending `file:line` when a raw output site, a plain `throw new Error(`, or an unsanctioned console/sink mutation appears.
