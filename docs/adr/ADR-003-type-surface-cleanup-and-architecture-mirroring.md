...
Clean up only the exported strict single-use bucket, in two passes: first **remove** the 145 declarations whose only use can be dropped through inference or equivalent narrowing while keeping `bun run check` clean; then **inline** the remaining inline-capable candidates whose declaration body can replace the single use. Prefer removal over inlining when inference preserves the same checking behavior. The `Only Imported/Used In One File, Multiple Uses` (257) and internal-reference (155) exported buckets are out of scope. Three no-import exported types (`ProviderManifestBase`, `RunTargetsOptionsBase`, `SecondsTimedTextRangeBase`) are **not** dead — each is a base extended within its own file — and are kept. Orphaned in-file helper/base types exposed by removals (e.g. `RunTargetsOptionsBase` once `RunTargetsOptions`/`RunSingleFileTargetsOptions` go) are cleaned up in the same batch. Fifteen in-scope candidates are also phase-2 parents (see §2); removing or inlining such a parent must inline/relocate its non-exported child in the same edit.
...
> Implementation Note: implemented. The exported strict single-use declarations were removed or inlined; the three kept bases (`ProviderManifestBase`, `RunTargetsOptionsBase` — itself later removed with its extenders — `SecondsTimedTextRangeBase`) behaved as documented, and a representative sweep confirms the removed names (`ArticleEstimateResult`, `FetchUrlResult`, `PreflightResult`, `UrlArticleResumePlan`, …) are gone from `src/types` while `bun run check` stays clean.
