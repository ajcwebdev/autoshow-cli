...
- `step-2-url/process-url.ts` — the public route coordinator delegating to the URL-local modules (compatibility re-exports retained for current callers/tests).
...
- Persisted metadata stays compatible: keep `resolvedStep2.route: "article"`, legacy `resolvedStep2.backend`/`backends`; add standardized `providers`, `requestedProviders`, `providerStates`, `missingProviders`; keep fixed `{ service, model, artifactDir, status, attempts }` run records and the existing artifact layout.
...
6. **There is no route-level version field.** The only discriminant is `schemaVersion: 2`; the existing gates for retired shapes (`assertSupportedRunOrBatchSchema`, `assertSupportedExtractBatchSchema` in `manifest-utils.ts`) are the pattern a route rename would need to extend, together with a `schemaVersion` bump.
...
- Shared registry types now include fixed `service === model` targets consumers must understand; legacy `resolvedStep2.backend`/`backends` and compatibility re-exports persist; URL execution stays structurally different from OCR/STT internals.
