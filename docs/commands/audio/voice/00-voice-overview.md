...
`comic reference-voice <action>` remains a deprecated compatibility alias for one release, with the same flags and behavior and a deprecation notice. Its bare form also runs `list`. See the [comic migration table](../../visuals/comic/00-comic-overview.md#deprecated-aliases) for replacements and the removal policy.
