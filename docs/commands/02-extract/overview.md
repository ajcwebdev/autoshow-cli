...
Control discovery and execution with `--batch-limit <n|all>`, `--batch-order newest|oldest`, and `--batch-concurrency <n>`. Mixed batches write a parent run directory with `media/`, `document/`, `article/`, and `x-space/` child directories for the routes used.
...
- [OCR extraction](./ocr/overview.md): document/image routing, hosted OCR engines, EPUB/PDF behavior, pricing, and OCR output notes.
- [URL and X extraction](./url/overview.md): remote article URLs, hosted article backends, X/Twitter Space inputs, and X output notes.
