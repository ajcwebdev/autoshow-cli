...
```bash
# Transcribe once after approving the provider and estimate.
...
# Export words with stable indices and a source-bound edit template.
...
# Caption the reviewed result. Use audioToVideoOffsetSeconds from raw/source-timeline.json (0 in this example).
