...
| `--qa-model <model>`                   | Vision judge model; QA currently requires an OpenAI vision-capable LLM                                                                         | `gpt-5.6-sol` |
...
- Multi-model runs write model-specific subdirectories under the run directory, such as `panels/<run-id>/<model>/`, `pages/<run-id>/<model>/`, and `sketches/<run-id>/<model>/`.
