...
`reference-voice` is the comic-native alias of [`voice`](../step-9-voice/00-voice-overview.md). It manages durable character voice registrations and never generates scene audio.
...
Actions match `voice`: `list`, `consent`, `import`, `design`, `clone`, `audition`, `approve`, `retire`, and `delete`. Bare `comic reference-voice` runs `list`. Clone is instant only; providers without an instant API use the provider console, then `comic reference-voice import --voice-id`. Run `bun autoshow comic reference-voice <action> --help` for the exact flags.
