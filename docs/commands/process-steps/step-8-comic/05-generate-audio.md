...
- Provider-admitted or ambiguous work without valid retained audio is blocked from automatic repurchase, and is never redispatched inside the running command. `--allow-ambiguous-redispatch` explicitly authorizes duplicate spend when a later run reconciles the stored slot and resumes.
