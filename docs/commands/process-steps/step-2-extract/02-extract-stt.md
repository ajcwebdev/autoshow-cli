...
- Before hosted provider upload, AutoShow stages one shared mono audio artifact with a 96 kbps ceiling, the original sample rate, and metadata stripped: AAC-LC `.m4a` when every hosted target is AssemblyAI, Deepgram, Gladia, Groq, Mistral, Rev, Soniox, or Speechmatics, and mp3 otherwise. Low-bitrate mono inputs stay on a stream-copy path. Supadata and ScrapeCreators use public source URLs directly.
