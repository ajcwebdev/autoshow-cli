...
| `--lyrics-file <path>`                | Lyrics file path (`.md` or `.txt`) with section headers; MiniMax and Gemini receive the lyrics directly, ElevenLabs builds a `music_v2` composition plan                          |
...
ElevenLabs returns audio directly (`mp3_48000_192` for `music_v2`). Pricing estimates use explicit `--duration` or default to 180 seconds. With `--lyrics-file`, the request switches to a `music_v2` composition plan: sections come from lyric headers such as `Verse 1` or `Chorus` (at most 30 sections), the prompt is parsed into style descriptors, and `--duration` or the 180-second default is distributed across the section chunks. `--instrumental` takes precedence over `--lyrics-file` and logs a warning.
...
Lyria 3 Pro appends a duration instruction to the prompt only when `--duration` is set; pricing and manifest metadata fall back to 120 seconds when it is omitted. `--lyrics-file` appends lyrics to the prompt. If `--instrumental` is also set, instrumental takes precedence and the lyrics file is ignored with a warning.
...
| ElevenLabs `music_v2`         | ✅ 2026-05-26 | ✅ 3–600s                         | ✅ `--duration`  | ✅ `--instrumental` | ✅ `--lyrics-file` composition plan | ✅ 48 kHz / 192 kbps MP3   | $0.15/min ($0.45 at the 180s default estimate) | 3/3       |
...
ElevenLabs sends `music_length_ms` only when `--duration` is set; omitted requests let the provider choose length while estimates still use 180 seconds, and a `--lyrics-file` request instead sends a composition plan whose chunk durations total `--duration` or that same default. MiniMax ignores `--duration`, auto-generates lyrics when `--lyrics-file` is omitted, and caps prompts at 2000 characters and lyrics at 3500 characters. Gemini instrumental mode wins over `--lyrics-file`. Lyric-video rendering is local and mutually exclusive with hosted generation flags. Direct selection of retired `music_v1` and `lyria-3-clip-preview` fails with replacement guidance to `music_v2` and `lyria-3-pro-preview`.
