...
| `--output-dir`       | `output/benchmark/<timestamp>` | Global flag: pin an exact output directory for benchmark files |
...
Text mode is no-cost and analysis-only. It reads completed `write` outputs and writes `provider-comparison-report.json` plus `provider-comparison-report.md`.
...
Image mode calls the OpenAI Responses API once per generated image. Treat `bun autoshow benchmark <image-run-dir> --image` as a paid provider run.
...
Video mode calls the OpenAI Responses API once per generated video after extracting local frame screenshots. Treat `bun autoshow benchmark <video-run-dir> --video` as a paid provider run.
...
Text reports split `llama.cpp` into the local group and hosted LLM providers into the service group. Price rankings use local zero monetary cost and reported actual or estimated service costs. Speed rankings prefer normalized `msPerUnit` timing when present, which lets the benchmark ranking report compare LLM speed across runs on a 1K-token basis.

Text quality is not inferred from length, speed, cost, output existence, schema validity, token count, model family, or subjective judgment. Automated and human quality rankings remain unavailable unless explicit future text quality fields exist.
...
```json
{
  ...
  "summary": {
    ...
    "serviceRankings": [
      ...
      { "service": "deepinfra", "model": "openai/whisper-large-v3-turbo", "averageWer": 0.042 }
    ]
  }
}
```
