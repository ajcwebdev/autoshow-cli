...
| `--panel-count <n>`                    | Require exactly this many panels from the `scene` stage, one per authored `[Panel N]` note in order, with one validator retry; only valid with `--only scene` or a full run | none                   |
...
- `--panel-count <n>` appends a panel count contract to the scene-drafting prompt: exactly `n` panels, and when the script authors `[Panel N]` notes, panel `k` must cite the `k`-th authored panel-note segment. The stage fails before any call when the authored note count differs from `n`, and a draft that breaks the contract gets one automatic retry that appends the validator issues, exactly like a blocking-plan contradiction. Treatments drafted by [`draft-treatment`](./07-draft-treatment.md) always author one note per panel, so pass the same count here.
...
- `--price` estimates the `blocking` stage as up to two calls at 3,000 output units each plus 1,000 modeled image input units per location establishing view, and the `scene` stage as 400 fixed output units plus 480 output units per estimated panel, doubled to two calls when a plan section or `--panel-count` contract is present; `--panel-count` also replaces the panel estimate. Import and rebind runs report zero calls.
