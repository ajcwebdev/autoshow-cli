...
| `--price` | Check panels, audio, and encoder readiness and report $0 without writes | `false` |
...
- [`resume <run-directory> --price` and `resume <run-directory>`](../../00-setup-and-utilities/resume.md#comic-recovery) can finish an interrupted local render, including a pending `generate-audio --slideshow` request. Missing or changed upstream media block recovery. Identical completed presentations are reused.
