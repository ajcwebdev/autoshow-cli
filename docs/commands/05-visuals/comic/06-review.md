...
With neither option, the command writes the HTML review sheet. Empty `--notes` paths and combining `--notes` with `--export-doc` are rejected. Neither mode rewrites the script or runs generation.
...
- Each panel section shows the source text, description, shot plan, characters, speech, the stage board when `metadata/blocking/panel-NN.svg` exists (or a no-plan placeholder), and the canonical `panels/panel-NN.png` when one is promoted.
- When QA evidence is present, the sheet shows the attempt count and QA summary. Otherwise it reads `QA evidence not retained`.
...
- Each note maps to its panel's first source beat and that beat's script line. When no line matches, the line is reported as unresolved instead of guessed.
- The command writes `metadata/review/review-notes-<run-id>.md`. It never rewrites the script.
...
`metadata/review/review-notes-<run-id>.md` includes a summary of mapped notes, unmatched notes when any failed to map, and one paste-ready directive per mapped note. The same directive lines are printed to the terminal.
...
- `**COSTUME:** {character: <key>}` — the character the note names when recognized, otherwise the panel's first character, otherwise `<character-key>`.
