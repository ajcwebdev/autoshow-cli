...
- The notes box under each panel collects into the exact `### Panel NN` format that [`comic review --notes`](#notes-processing) reads back, so a review pass round-trips without retyping panel numbers.
