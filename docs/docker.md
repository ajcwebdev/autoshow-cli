...
Each image leg is inventoried once with Syft and audited once with Grype through the shared `.github/actions/scan-image` composite, which restores a Grype vulnerability database cached per UTC day across jobs and workflows and downloads a fresh database only on the first run of each day. The fixable-high gate is derived from that single scan with `jq` (`fixable.jq` emulates `--only-fixed` and `gate-count.jq` applies the severity cutoff) instead of a second Grype invocation. Every evidence directory receives `image.spdx.json`, `image-advisories.json`, `image-advisories-fixable.json`, and `grype-db.json`, and the release advisory triage is still recorded from `image-advisories.json` when the gate fails.
