...
AutoShow publishes a Docker image with the CLI and common local tools so you can run without installing Bun or those tools on the host. Pre-built `linux/amd64` and `linux/arm64` images are on GitHub Container Registry (GHCR), tagged `latest` and by full commit SHA; they are published for every push to `main` that changes image inputs (`Dockerfile`, dependencies, `config/`, `src/`, the acceptance harness, or workflows), and docs-only pushes keep the previous `latest`.
...
| `local/sources/download/download-input-types-streaming`: Twitch                                                                           | `download-twitch`; same public URL, success required                                                                                                                                                       |
...
The publishing workflow starts no-cost verification, package hygiene, and the native AMD64 and ARM64 image builds together instead of serializing them; each build pushes an untagged per-architecture digest, and three evidence jobs per architecture (`smoke-<arch>`, `compiled-experiment-<arch>`, and `audit-<arch>`) pull that digest concurrently to run the native smoke and measurement pass, the measured compiled-entrypoint experiment, and the SBOM, advisory audit, and triage gate. `publish-manifest` requires verification, hygiene, both builds, smoke, and audit, so `latest` and the `:sha` tag are never created unless those gates succeed. The compiled-entrypoint experiment still runs and uploads evidence, but it is advisory and does not block tagging. After the multi-architecture manifest is published, 12 native acceptance jobs (2 architectures × 6 shards: core, public-network, and one shard per whisperfile model) on `ubuntu-24.04` and `ubuntu-24.04-arm` pull `latest`, check `--expected-digest` and `--expected-revision` against publication outputs, and report to the `Published latest acceptance` aggregate check. Evidence artifacts are `docker-smoke-<arch>`, `docker-compiled-experiment-<arch>`, `docker-audit-<arch>`, and `ghcr-acceptance-<arch>-<index>`; artifacts and local asset caches are saved on failure, and CI restores host ownership before archiving cache directories, including private Calibre configuration directories. The workflow holds a workflow-level concurrency lock from verification through acceptance with cancellation disabled, so push runs are never cancelled and a concurrent release cannot replace `latest` during validation. There is no rollback or retagging, and this workflow check does not itself change repository branch-protection settings. GitHub documents [workflow concurrency](https://docs.github.com/en/actions/how-tos/write-workflows/choose-when-workflows-run/control-workflow-concurrency).

Each architecture build imports and exports a registry layer cache at `ghcr.io/ajcwebdev/autoshow-cli:buildcache-amd64` or `:buildcache-arm64` (`mode=max`). The Dockerfile declares `AUTOSHOW_VERSION`, `BUILD_DATE`, and `VCS_REF` after the last `RUN` of each runtime target, so per-commit build identity never invalidates the pinned `DEBIAN_SNAPSHOT` apt layer; only a Dockerfile, dependency, or source change rebuilds it. The independent dependency-graphs workflow reuses the same per-architecture cache on pull requests and pushes, and rebuilds the audit images with `--pull --no-cache` on its weekly Monday schedule or when dispatched with `fresh_build`, so a stale cache is bounded to one week. Acceptance jobs restore local model and tool assets from a cache keyed by architecture, shard, and a content hash, and save a new entry only when nothing was restored.
...
Each image leg is inventoried once with Syft and audited once with Grype through the shared `.github/actions/scan-image` composite, which restores a Grype vulnerability database cached per UTC day across jobs and workflows and downloads a fresh database only on a cache miss. Actions cache entries are branch-scoped, so a nightly `Warm Grype vulnerability database` workflow on `main` hydrates today's database from a tiny SPDX document (`fail-on-fixable: false`) and makes that entry visible to later `main` pushes and pull requests. The fixable-high gate is derived from that single scan with `jq` (`fixable.jq` emulates `--only-fixed` and `gate-count.jq` applies the severity cutoff) instead of a second Grype invocation. Every evidence directory receives `image.spdx.json`, `image-advisories.json`, `image-advisories-fixable.json`, and `grype-db.json`, and the release advisory triage is still recorded from `image-advisories.json` when the gate fails.
