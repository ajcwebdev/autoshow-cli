...
YouTube can require sign-in from GitHub-hosted runners even when the same anonymous download works locally. To authenticate that one acceptance case, supply a dedicated Netscape cookie file using `bun t:docker --suite network --youtube-cookies /absolute/path/to/youtube.cookies.txt`. Keep the file outside evidence and cache directories. The harness filters out other domains and mounts a temporary snapshot read-only into only the registered YouTube download container. A shell launcher makes a writable cookie copy and config in that container's private temporary directory, then executes the image's normal entrypoint as its normal user. Authenticated yt-dlp caches also stay in the container. The source file is unchanged, and cookie contents never enter the image, shared asset cache, or uploaded evidence.
