...
function usage(): string {
  return [
    "Usage:",
    "  bun ocr_batch_queue.ts next --root <dir> [--state <path>] [--recursive] [--ext <ext>] [--size <n>]",
    "  bun ocr_batch_queue.ts done --root <dir> [--state <path>] [--recursive] [--ext <ext>] <file> [file...]",
    "  bun ocr_batch_queue.ts status --root <dir> [--state <path>] [--recursive] [--ext <ext>] [--size <n>]",
    "  bun ocr_batch_queue.ts reset --root <dir> [--state <path>] [--recursive] [--ext <ext>]",
  ].join("\n");
}
...
function defaultStatePath(root: string): string {
  return resolve(root, ".ocr-batch-state.json");
}
...
function listSourceFiles(root: string, recursive: boolean, ext: string): string[] {
  const pattern = recursive ? `**/*.${ext}` : `*.${ext}`;
...
function parseShared(command: ParsedArgs["command"], argv: string[]): SharedArgs & { size: number; files: string[] } {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--ext") {
      ...
      ext = normalizeExt(value);
      if (!ext) {
        throw new Error("--ext must be a non-empty file extension");
      }
      index += 1;
      continue;
    }
...
function parseArgs(argv: string[]): ParsedArgs {
  if (argv.includes("--help") || argv.includes("-h")) {
    console.log(
      [
        usage(),
        "",
        "Track single-file batch processing state for OCR fidelity-cleanup passes.",
        "",
        "Commands:",
        ...
        "",
        "Options:",
        "  --root <dir>     Root directory containing source files (required)",
        "  --state <path>   Custom state file path (default: <root>/.ocr-batch-state.json)",
        "  --recursive      Include source files in subdirectories",
        "  --ext <ext>      Source file extension to scan (default: txt; e.g. md)",
  ...
  if (command === "next") {
    return { command, root: shared.root, state: shared.state, recursive: shared.recursive, ext: shared.ext, size: shared.size };
  }
  if (command === "done") {
    return { command, root: shared.root, state: shared.state, recursive: shared.recursive, ext: shared.ext, files: shared.files };
  }
  if (command === "status") {
    return { command, root: shared.root, state: shared.state, recursive: shared.recursive, ext: shared.ext, size: shared.size };
  }
  return { command, root: shared.root, state: shared.state, recursive: shared.recursive, ext: shared.ext };
}

async function cmdNext(args: NextArgs): Promise<number> {
  const statePath = resolveStatePath(args.root, args.state);
  const allFiles = listSourceFiles(args.root, args.recursive, args.ext);
...
async function cmdDone(args: DoneArgs): Promise<number> {
  const statePath = resolveStatePath(args.root, args.state);
  const state = await loadState(statePath);
  const allFiles = new Set(listSourceFiles(args.root, args.recursive, args.ext));
