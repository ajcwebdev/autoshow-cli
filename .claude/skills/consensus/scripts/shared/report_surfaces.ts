  ...
  for (const record of records) {
    ...
    const summary: ProviderSummary = {
      ...
      metrics: {
        ...
        outputTokenCount: asNumber(record.outputTokenCount) ?? asNumber(nestedMetrics.outputTokenCount),
        totalTokenCount: asNumber(record.totalTokenCount) ?? asNumber(nestedMetrics.totalTokenCount),
        msPerUnit: firstNumber(record, ["msPerUnit"]) ?? asNumber(nestedMetrics.msPerUnit),
        ...
        automatedQualityScore: asNumber(record.automatedQualityScore) ?? asNumber(nestedMetrics.automatedQualityScore),
        textQualityScore: asNumber(record.textQualityScore) ?? asNumber(nestedMetrics.textQualityScore),
...
function qualityEvidence(category: ConsensusCategory, provider: ProviderSummary): QualityEvidence | null {
  ...
  if (category === "text" && (metrics.automatedQualityScore !== null || metrics.textQualityScore !== null)) {
    const score = metrics.automatedQualityScore ?? metrics.textQualityScore ?? 0;
    return {
      kind: "higher-is-better",
      metric: "explicit text quality score",
      value: score,
      label: `${score.toFixed(2)} explicit text quality score`,
      tieBreakers: [],
    };
  }
  return null;
...
function formatSpeed(provider: ProviderSummary): string {
  return provider.msPerUnit === null
    ? formatDurationMs(provider.processingTimeMs)
    : `${provider.msPerUnit.toFixed(3)} ms/1K tokens`;
}
...
function fullSpeedRanking(providers: ProviderSummary[], group: ProviderGroup): { entries: RankingEntry[]; reason: string | null } {
  ...
  const entries = [...providers]
    .sort((left, right) =>
      compareNullableAscending(left.msPerUnit ?? left.processingTimeMs, right.msPerUnit ?? right.processingTimeMs) || left.providerKey.localeCompare(right.providerKey)
    )
    .map((provider, index) => surfaceEntry(
      provider,
      index + 1,
      provider.msPerUnit === null ? "processingTimeMs" : "msPerUnit",
      provider.msPerUnit ?? provider.processingTimeMs,
      formatSpeed(provider),
    ));
...
function automatedQualityReason(category: ConsensusCategory, group: ProviderGroup): string {
  ...
  if (category === "text") {
    return `No explicit text quality score was available for ${group} providers. Length, speed, cost, output existence, schema validity, and subjective judgment are not used as automated quality proxies.`;
  }
...
function fullAutomatedQualityRanking(
  ...
  if (category === "text") {
    const withQualityScore = providers.filter((provider) => provider.metrics.automatedQualityScore !== null || provider.metrics.textQualityScore !== null);
    if (withQualityScore.length === 0) {
      return { entries: [], reason: automatedQualityReason(category, group) };
    }
    const entries = withQualityScore
      .sort((left, right) => {
        const leftScore = left.metrics.automatedQualityScore ?? left.metrics.textQualityScore ?? Number.NEGATIVE_INFINITY;
        const rightScore = right.metrics.automatedQualityScore ?? right.metrics.textQualityScore ?? Number.NEGATIVE_INFINITY;
        return rightScore - leftScore || left.providerKey.localeCompare(right.providerKey);
      })
      .map((provider, index) => {
        const score = provider.metrics.automatedQualityScore ?? provider.metrics.textQualityScore ?? 0;
        return surfaceEntry(provider, index + 1, "explicit text quality score", score, `${score.toFixed(2)} explicit text quality score`);
      });
    return { entries, reason: null };
  }
...
function humanQualityReason(category: ConsensusCategory, group: ProviderGroup): string {
  ...
  if (category === "text") {
    return `No explicit humanQualityScore was available for ${group} providers. Length, speed, cost, output existence, schema validity, and subjective judgment are not used as human quality proxies.`;
  }
...
function providerDetail(category: ConsensusCategory, provider: ProviderSummary): Record<string, unknown> {
  ...
  if (category === "text") {
    detail.inputTokenCount = provider.source.inputTokenCount ?? provider.metrics.inputTokenCount ?? null;
    detail.outputTokenCount = provider.source.outputTokenCount ?? provider.metrics.outputTokenCount ?? null;
    detail.totalTokenCount = provider.source.totalTokenCount ?? provider.metrics.totalTokenCount ?? null;
    detail.tokenCountSource = provider.source.tokenCountSource ?? null;
    detail.providerUsage = provider.source.providerUsage ?? null;
    detail.rawProviderUsage = provider.source.rawProviderUsage ?? null;
    detail.outputFileName = provider.source.outputFileName ?? null;
    detail.outputExists = provider.source.outputExists ?? null;
    detail.outputByteSize = provider.source.outputByteSize ?? null;
    detail.structuredMode = provider.source.structuredMode ?? null;
    detail.structuredPresetNames = provider.source.structuredPresetNames ?? [];
  }
  if (category === "url") {
  ...
  if (category === "ocr" || category === "stt") {
    ...
    return {
      ...
      ...(category === "ocr" && sourceReport.consensusSkillArtifacts
        ? { consensusSkillArtifacts: sourceReport.consensusSkillArtifacts }
        : {}),
...
function textDetailTable(providers: ProviderSummary[], group: ProviderGroup): string {
  if (providers.length === 0) {
    return `No ${group} providers were found.`;
  }
  return [
    ...
    "| --- | ---: | ---: | ---: | --- | --- |",
    ...providers.map((provider) => {
      const inputTokens = provider.metrics.inputTokenCount ?? 0;
      const outputTokens = provider.metrics.outputTokenCount ?? 0;
      const outputFile = asString(provider.source.outputFileName);
      const outputExists = provider.source.outputExists === false ? " (missing)" : "";
      return markdownRow([
        providerMarkdownCode(provider),
        `${inputTokens.toLocaleString()} in / ${outputTokens.toLocaleString()} out`,
        formatSpeed(provider),
        group === "local" ? "$0.00" : formatCents(provider.costCents),
        outputFile ? `${outputFile}${outputExists}` : "n/a",
        provider.quality?.label ?? "n/a",
      ]);
    }),
  ].join("\n");
...
function groupMarkdown(category: ConsensusCategory, label: string, group: ProviderGroup, providers: ProviderSummary[], surfaces: SurfaceGroup): string {
  return [
    `## ${label} Providers`,
    "",
    ...
    "### Provider Detail",
    "",
    category === "text" ? textDetailTable(providers, group) : detailTable(providers, group),
  ].join("\n");
}
...
function buildMarkdownReport(
  ...
  return [
    ...
    "",
    groupMarkdown(category, "Local", "local", localProviders, rankingSurfaces.local),
    "",
    groupMarkdown(category, "Service", "service", serviceProviders, rankingSurfaces.service),
    "",
    "## Notes",
    "",
    notesBlock,
