...
export function buildPacket(runDir: string) {
  ...
  const pageAnalysis = buildOcrPageAnalysisArtifacts(runDir, providers);
  ...
  return {
    ...
    pages,
    pageAnalysis: {
      pageMetrics: pageAnalysis.pageMetrics,
      outliers: pageAnalysis.outliers,
      selectiveAdjudicationPages: pageAnalysis.selectiveAdjudicationPages,
      variantComparisonSummary: pageAnalysis.variantComparisonSummary,
    },
    warnings,
  };
}
