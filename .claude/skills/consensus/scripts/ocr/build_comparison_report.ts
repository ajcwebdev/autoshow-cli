...
interface PreviousReportProvider {
  providerKey?: unknown;
  provider?: unknown;
  model?: unknown;
  group?: unknown;
  ...
  werBreakdown?: Partial<Record<keyof WerBreakdownData, unknown>>;
  cerBreakdown?: Partial<Record<keyof WerBreakdownData, unknown>>;
}

interface PreviousReport {
  providerGroups?: {
    local?: { providers?: PreviousReportProvider[] };
    thirdPartyService?: { providers?: PreviousReportProvider[] };
  };
}
...
function helpText(): string {
  return [
    ...
    "  --previous-report <path> Read prior scored providers from this report before adding discovered providers",
...
function parseArgs(argv: string[]): ParsedArgs {
  ...
  let previousReportPath: string | null = null;
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--previous-report") {
      const value = argv[index + 1];
      if (!value) {
        throw new Error("Missing value for --previous-report");
      }
      previousReportPath = resolve(value);
      index += 1;
      continue;
    }
    if (arg === "--markdown-out") {
...
function previousProviderGroup(rawGroup: unknown, fallbackGroup: ProviderGroup): ProviderGroup {
  const group = asString(rawGroup)?.replace(/[-_\s]/g, "").toLowerCase();
  return group === "local" ? "local" : fallbackGroup;
}

function previousBreakdown(value: unknown): WerBreakdownData {
  const record = isRecord(value) ? value : {};
  return {
    substitutions: asNumber(record.substitutions) ?? 0,
    deletions: asNumber(record.deletions) ?? 0,
    insertions: asNumber(record.insertions) ?? 0,
    referenceCount: asNumber(record.referenceCount) ?? 0,
  };
}

function previousProviderData(
  provider: PreviousReportProvider,
  fallbackGroup: ProviderGroup,
): ProviderData | null {
  const providerKey = asString(provider.providerKey);
  const providerName = asString(provider.provider);
  const model = asString(provider.model);
  if (!providerKey || !providerName || !model) {
    return null;
  }

  const metrics = isRecord(provider.metrics) ? provider.metrics : {};
  const score = asNumber(provider.score) ?? asNumber(metrics.score);
  const wer = asNumber(provider.wer) ?? asNumber(metrics.wer);
  const cer = asNumber(provider.cer) ?? asNumber(metrics.cer);
  if (score === null || wer === null || cer === null) {
    return null;
  }

  const group = previousProviderGroup(provider.group, fallbackGroup);
  return {
    providerKey,
    ...
    wer,
    cer,
    werBreakdown: previousBreakdown(provider.werBreakdown),
    cerBreakdown: previousBreakdown(provider.cerBreakdown),
    tokenEstimate: asNumber(provider.tokenEstimate),
    processingTimeMs: asNumber(provider.processingTimeMs),
    costCents: group === "local" ? 0 : asNumber(provider.costCents),
  };
}

function loadPreviousProviderData(reportPath: string | null, currentProviderKeys: Set<string>): ProviderData[] {
  if (!reportPath || !existsSync(reportPath)) {
    return [];
  }

  const parsed = JSON.parse(readFileSync(reportPath, "utf8")) as PreviousReport;
  const providers = [
    ...(parsed.providerGroups?.local?.providers ?? []).map((provider) => previousProviderData(provider, "local")),
    ...(parsed.providerGroups?.thirdPartyService?.providers ?? []).map((provider) => previousProviderData(provider, "cloud")),
  ].filter((provider): provider is ProviderData => provider !== null);

  return providers.filter((provider) => !currentProviderKeys.has(provider.providerKey));
}

export function buildReport(runDir: string, consensusPath: string, previousReportPath: string | null = resolve(runDir, "provider-comparison-report.json")) {
  ...
  const currentProviderKeys = new Set(providerData.map((provider) => provider.providerKey));
  const preservedProviderData = loadPreviousProviderData(previousReportPath, currentProviderKeys);
  if (preservedProviderData.length > 0) {
    warnings.push(
      `Preserved ${preservedProviderData.length} prior provider report entries without local result.json files from ${previousReportPath}`,
    );
    providerData.unshift(...preservedProviderData);
  }
  ...
  const analysisArtifacts = buildOcrPageAnalysisArtifacts(runDir, providers, consensus.pages);
  ...
  const reportJson = {
    ...
    werFormula: "(Substitutions + Deletions + Insertions) / Reference Word Count",
	    ...
	    providerGroups,
	    metricRankings,
      consensusSkillArtifacts: {
        pageMetrics: "page-metrics.json",
        outliers: "outliers.json",
        selectiveAdjudicationPages: "selective-adjudication-pages.json",
        variantComparisonSummary: "variant-comparison-summary.json",
        benchmarkSummary: "ocr-benchmark-summary.md",
      },
	    notes,
	  };
...
`;
  ...
  return { reportJson, markdown, warnings, analysisArtifacts };
}
...
function main(): number {
  ...
  const { reportJson, markdown, warnings, analysisArtifacts } = buildReport(
    args.runDir,
    consensusPath,
    args.previousReportPath ?? jsonOut,
  );
