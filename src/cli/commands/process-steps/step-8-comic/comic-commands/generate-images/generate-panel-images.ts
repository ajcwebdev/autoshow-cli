          ...
          for (const variation of variations) {
            for (const model of options.models) {
              ...
              if (outputExists && !qaEntry) {
                qaEntry = await judge({ pageNumber: panelNumber, pagePath: outputPath, panelData: bundleData, identityCards: resolvedReferences.primaryCharacterRefs, locationSheets: resolvedReferences.secondaryRefs, designSheets: resolvedReferences.designReferences?.map(reference => reference.path), characterReferences: resolvedReferences.characterReferences, locationReferences: resolvedReferences.locationReferences, designReferences: resolvedReferences.designReferences, model: judgeModel })
