          ...
          for (const variation of variations) {
            for (const model of options.models) {
              ...
              if (outputExists && !qaEntry) {
                qaEntry = await judge({ pageNumber: panelNumber, pagePath: outputPath, panelData: bundleData, identityCards: resolvedReferences.primaryCharacterRefs, locationSheets: resolvedReferences.secondaryRefs, characterReferences: resolvedReferences.characterReferences, locationReferences: resolvedReferences.locationReferences, model: judgeModel })
              ...
              for (let attempt = firstAttempt; attempt <= maxRepairs; attempt++) {
                ...
                try {
                  qaEntry = await judge({ pageNumber: panelNumber, pagePath: attemptPath, panelData: bundleData, identityCards: resolvedReferences.primaryCharacterRefs, locationSheets: resolvedReferences.secondaryRefs, characterReferences: resolvedReferences.characterReferences, locationReferences: resolvedReferences.locationReferences, model: judgeModel })
                  stats.totalInputTokens += qaEntry.usage.inputTokens
                  stats.totalOutputTokens += qaEntry.usage.outputTokens
                  stats.totalCost += qaEntry.usage.costUsd
                } catch (error) {
