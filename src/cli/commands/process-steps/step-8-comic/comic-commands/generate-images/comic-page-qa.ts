...
export type PageQaResult = {
  ...
  panels: Array<{
    ...
    setContinuityAudit?: Array<{
      anchor: string
      status: 'present-correctly' | 'outside-crop' | 'physically-occluded' | 'missing' | 'relocated' | 'duplicated' | 'mirrored' | 'redesigned'
      evidence: string
    }>
...
export const parseComicPageQaResult = (text: string, expectedPanels: number[]): PageQaResult => {
  ...
  for (const panel of result.panels) {
    ...
    const validContinuityStatuses = ['present-correctly', 'outside-crop', 'physically-occluded', 'missing', 'relocated', 'duplicated', 'mirrored', 'redesigned']
...
export const hasHardPageQaFailure = (result: PageQaResult, options: { waiveShotPlanMatch?: boolean } = {}): boolean =>
  !result.panelStructure.pass || result.panels.some(panel =>
    !panel.requiredCastPresent || !panel.unexpectedCastAbsent || panel.identityIssueKind === 'unmistakable-mismatch' || (panel.identityIssueKind === undefined && !panel.identityMatch) || panel.locationMatch === false || panel.setContinuityMatch === false || panel.setContinuityAudit?.some(item => ['missing', 'relocated', 'duplicated', 'mirrored', 'redesigned'].includes(item.status)) === true || panel.sourcePrecedence === false || (!options.waiveShotPlanMatch && panel.shotPlanMatch === false) || panel.dialogueIssueKind === 'content' || (panel.dialogueIssueKind === undefined && !panel.dialogueAccuracy) || !panel.speakerAttribution
  )
