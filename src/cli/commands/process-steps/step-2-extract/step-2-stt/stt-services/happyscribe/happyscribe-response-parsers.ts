import type { HappyScribeExport, HappyScribeOrder, HappyScribeTranscription } from '~/types'
...
export const parseHappyScribeOrder = (payload: unknown): HappyScribeOrder => {
  if (!isRecord(payload)) {
    throw ValidationError('Happy Scribe order response was not an object', { stage: 'stt:happyscribe' })
  }

  const id = normalizeHappyScribeId(payload['id'])
  if (!id || typeof payload['state'] !== 'string') {
    throw ValidationError('Happy Scribe order response missing id or state', { stage: 'stt:happyscribe' })
  }
...
export const parseHappyScribeTranscription = (payload: unknown): HappyScribeTranscription => {
  if (!isRecord(payload)) {
    throw ValidationError('Happy Scribe transcription response was not an object', { stage: 'stt:happyscribe' })
  }
...
export const parseHappyScribeExport = (payload: unknown): HappyScribeExport => {
  if (!isRecord(payload)) {
    throw ValidationError('Happy Scribe export response was not an object', { stage: 'stt:happyscribe' })
  }

  const id = normalizeHappyScribeId(payload['id'])
  if (!id || typeof payload['state'] !== 'string') {
    throw ValidationError('Happy Scribe export response missing id or state', { stage: 'stt:happyscribe' })
  }
