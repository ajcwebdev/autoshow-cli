...
const logReverbTokenNextSteps = (): void => {
  ...
  l.write('warn', 'Reverb Setup Next Steps', {
    ...
    humanTable: createHumanTable([
      ...
      { step: 3, command: 'bun autoshow setup --step reverb' }
    ], ['step', 'command'])
  })
}
...
const runReverbSetupPhase = async (
  ...
  try {
    ...
    throw InfraError(`Reverb setup failed during ${phase}: ${formatInstallError(error)}`, { stage: 'stt:reverb-setup', cause: error instanceof Error ? error : undefined })
  }
}
...
export const setupReverb = async (): Promise<void> => {
  ...
  if (!await checkReverbModelExists()) {
    if (!token) {
      ...
      throw InternalError('Missing Hugging Face token for Reverb model download', { stage: 'stt:reverb-setup', hints: hintsForMissingEnv('HUGGINGFACE_TOKEN') })
    }
    ...
    try {
      ...
      throw InfraError('Failed to download Reverb model', { stage: 'stt:reverb-setup' })
    }
  } else {
  }
  ...
  if (isCompactSetupMode()) {
    ...
    l.write('success', 'Reverb ASR Setup', {
      category: 'command',
      humanTable: createHumanTable([
        { status: 'complete', command: 'bun autoshow extract "URL" --reverb' },
        { status: 'complete', command: 'bun autoshow extract "URL" --reverb --stt-reverb-verbatimicity 0.5' }
      ], ['status', 'command'])
    })
  }
}
...
export const ensureReverbRuntimeSetup = async (): Promise<void> => {
  ...
  if (!await checkReverbModelExists()) {
    try {
      ...
      throw InfraError('Failed to ensure Reverb model setup', { stage: 'stt:reverb-setup' })
    }
  }
}
