...
export const runReverbTranscribe = async (
  ...
  try {
    ...
    const result = await exec(uvCommand, args, {
      retry: { operationName: 'Reverb transcription' }
    })
    if (result.stderr) {
      ...
      if (hasError && result.exitCode !== 0) {
        throw InfraError(`Reverb transcription failed with errors`, { stage: 'stt:reverb' })
      }
    }
    if (result.stdout && result.stdout.includes('Traceback')) {
      ...
      throw InfraError('Reverb transcription failed with Python error', { stage: 'stt:reverb' })
    }
    if (result.exitCode !== 0) {
      l.error(`Reverb transcription failed with exit code ${result.exitCode}`)
      throw InfraError(`Reverb transcription failed with exit code ${result.exitCode}`, { stage: 'stt:reverb' })
    }
      ...
      if (ctmPath) {
        ...
        if (rttmPath) {
          ...
          if (diarizedData && typeof diarizedData === 'object' && diarizedData !== null) {
            ...
            if (!textContent) throw InfraError('Reverb transcription produced no readable output', { stage: 'stt:reverb' })
