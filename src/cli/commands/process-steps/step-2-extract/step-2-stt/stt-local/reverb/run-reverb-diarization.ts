...
import { checkReverbDiarizationAssets, getMissingReverbDiarizationFiles, reverbDiarizationConfigPath } from './reverb-assets'
import type { ReverbDiarizationModel } from '~/types'
...
export const resolveDiarizationModel = async (): Promise<ReverbDiarizationModel | null> => {
  ...
  if (!hfToken) {
    ...
    l.warn(`Reverb diarization assets are missing (${missing.join(', ')}). Set HUGGINGFACE_TOKEN and run \`bun autoshow setup --step reverb\` to download them.`)
    return null
  }
