...
import { decodeXmlEntities, firstStartTag, firstTagAttr, firstTagBlock, firstTagText, innerXml, readAttr, scanTagBlocks } from '~/utils/xml-scan'
import type { EpubAssets, EpubChapter, EpubContentReader, EpubInspectEngine, EpubInspectionPayload, EpubInspectOutput, EpubManifestItem, EpubMetadata, EpubTocItem, TocBoundary } from '~/types'
...
const normalizeRootRelativeHref = (href: string): string => {
  const cleanHref = href.split(/[?#]/, 1)[0]?.trim() ?? ''
  if (!cleanHref) return ''
...
const readEncryptedEntryPaths = async (reader: EpubContentReader): Promise<Set<string>> => {
...
const validateReadableSpineContent = (html: string, spinePath: string): void => {
