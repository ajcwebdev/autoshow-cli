...
const TOC_ARTIFACT_RE = /\b(?:https?:\/\/|www\.|web\.archive|archive\.org|internet archive|digitized by|library of congress|university library|university of [a-z ]+ library|isbn|catalog(?:ing)?|copyright|all rights reserved|cover design|back cover|title page|original page|orignal page|main branch|printing|\d{1,2}\/\d{1,2}\/\d{2,4})\b/i
...
export const isTocHeaderLine = (value: string): boolean => {
  const normalized = normalizeTitle(value)
  return /^(?:chapter|chapters) (?:page|pages)$/.test(normalized)
    || /^(?:page|pages) (?:chapter|chapters)$/.test(normalized)
}

export const stripTocHeaderPrefix = (value: string): string => {
  const normalized = normalizeInlineWhitespace(value)
  if (isTocHeaderLine(normalized)) {
    return ''
  }
  return normalized
    .replace(/^(?:chapter|chapters)\s+(?:page|pages)\s+/i, '')
    .replace(/^(?:page|pages)\s+(?:chapter|chapters)\s+/i, '')
    .trim()
}

export const stripTocDotLeaders = (value: string): string =>
  normalizeInlineWhitespace(value.replace(/\s*(?:[.·]\s*){3,}/g, ' '))

const EXPLICIT_NUMBERED_CHAPTER_START_RE = /(?:(?:chapter|chap\.?|part|book|section)\s+[0-9ivxlcdm]+(?:\s*[.:])?|\d{1,4}\s*[.)?:-]|[ivxlcdm]{1,10}\s*[.)?:-])\s+\S/ig
const RUNNING_HEADER_SEPARATOR_RE = /(?:\/|\||[-–—])/
const RUNNING_HEADER_PAGE_LABEL_RE = /(?:^|\s)(?:\d{1,4}|[ivxlcdm]{1,10})(?:\s|$)/i
const HIERARCHICAL_TITLE_PREFIX_RE = /^(?:(?:book|part|chapter|chap\.?|section|volume|vol\.?)\s+[0-9ivxlcdm]+(?:\s*(?:\/|\||[-–—:])\s*)?)+$/i

const isHierarchicalTitlePrefix = (value: string): boolean =>
  HIERARCHICAL_TITLE_PREFIX_RE.test(normalizeInlineWhitespace(value))

const isLikelyRunningHeaderPrefix = (value: string): boolean => {
  const normalized = normalizeInlineWhitespace(value)
  return normalized.length > 0
    && normalized.length <= 90
    && RUNNING_HEADER_SEPARATOR_RE.test(normalized)
    && RUNNING_HEADER_PAGE_LABEL_RE.test(normalized)
    && !isHierarchicalTitlePrefix(normalized)
    && countAlphaChars(normalized) >= 2
}

const stripLeadingRunningHeaderFromTitle = (value: string): string => {
  EXPLICIT_NUMBERED_CHAPTER_START_RE.lastIndex = 0
  let match: RegExpExecArray | null
  while ((match = EXPLICIT_NUMBERED_CHAPTER_START_RE.exec(value)) !== null) {
    if (match.index <= 0) {
      continue
    }
    const prefix = value.slice(0, match.index)
    if (isLikelyRunningHeaderPrefix(prefix)) {
...
export const cleanDetectedChapterTitle = (value: string): string => {
  const normalized = stripTocDotLeaders(stripTocHeaderPrefix(value))
  const withoutLeadingHeader = stripLeadingRunningHeaderFromTitle(normalized)
  return withoutLeadingHeader.length > 0 ? withoutLeadingHeader : normalized
}
...
export const getOutlineRejectReason = (title: string): string | undefined => {
  const cleanedTitle = stripTocHeaderPrefix(title)
  ...
  if (isLikelyFilenameTitle(cleanedTitle)) {
...
export const isLikelyNoisyTocTitle = (
  ...
  const cleanedValue = stripTocHeaderPrefix(value)
  const normalized = normalizeTitle(cleanedValue)
  if (normalized.length === 0) {
    return true
  }
  if ([
  ...
  if (isLikelyFilenameTitle(cleanedValue) || isLikelyArtifactText(cleanedValue)) {
    return true
  }

  const wordCount = countWords(cleanedValue)
  const numericTokens = countNumericTokens(cleanedValue)
...
export const buildChapterSlug = (title: string, pdfStartPage: number): string => {
  const slug = sanitizeTitleSlug(cleanDetectedChapterTitle(title), CHAPTER_SLUG_MAX_LENGTH)
