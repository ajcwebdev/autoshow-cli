...
import type { AudioNormalizationMode, AudioNormalizationProfile, AudioStreamProbe, FfprobeFormat, FfprobePayload, FfprobeStream, MediaProbe, NormalizedAudioExtension, NormalizedAudioFormat, NormalizedAudioPlan } from '~/types'
...
export const probeMediaFile = async (inputPath: string): Promise<MediaProbe> => {
  ...
  if (result.exitCode !== 0) {
    throw InfraError(`Failed to probe media file: ${result.stderr || result.stdout}`, { stage: 'download:audio' })
  }
  ...
  if (!audioStreamRaw || typeof audioStreamRaw.codec_name !== 'string') {
    throw InfraError(`No audio stream found in ${inputPath}`, { stage: 'download:audio' })
  }
