...
const normalizeDownloadedAudio = async (
  ...
  if (fileSize < 1000) {
    throw InfraError(`Input file is too small (${fileSize} bytes), likely corrupted`, { stage: 'download:audio' })
  }
...
const downloadDirectAudioUrl = async (url: string, outputDir: string): Promise<string> => {
  ...
  if (!resp.ok) {
    throw InfraError(`Direct download failed: ${resp.status} ${resp.statusText} (${url})`, { stage: 'download:audio' })
  }

  const contentType = resp.headers.get('content-type') ?? ''

  if (contentType.startsWith('text/html')) {
    throw InfraError(`Expected audio from ${url}, got HTML (content-type: ${contentType})`, { stage: 'download:audio' })
  }
  ...
  if (rawFile.size < 1000) {
    throw InfraError(`Downloaded file is too small (${rawFile.size} bytes), likely corrupted`, { stage: 'download:audio' })
  }
...
export const downloadAudio = async (options: DownloadAudioOptions, videoMetadata: VideoMetadata): Promise<{ audioPath: string, metadata: Step1Metadata }> => {
  ...
  if (!hasRuntimeTool('ffmpeg') || !hasRuntimeTool('ffprobe')) {
  ...
  if (options.filePath) {
    ...
    if (downloadedFile.size < 1000) {
      throw InfraError('Downloaded file is empty or corrupted', { stage: 'download:audio' })
    }
    audioPath = preserveOriginalMedia
