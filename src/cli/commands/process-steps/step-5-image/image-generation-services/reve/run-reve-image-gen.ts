...
import type { ReveDimensions, ReveImageModel, ReveOutputFormat, Step5Metadata } from '~/types'
...
export const REVE_ASPECT_RATIOS = ['16:9', '9:16', '3:2', '2:3', '4:3', '3:4', '1:1'] as const
export const REVE_OUTPUT_FORMATS = ['png', 'jpeg', 'webp'] as const
...
export const runReveImageGen = async (
  ...
  const response = await fetch(`${getReveBaseUrl(options.baseUrl)}${endpoint}`, {
    method: 'POST',
    headers: {
  ...
  if (errorCode || contentViolation) {
    throw InfraError(`Reve image generation failed: ${contentViolation ? 'content violation' : `error code ${errorCode}`}`, { stage: 'image:reve' })
  }
  if (!response.ok) {
    throw InfraError(await buildReveErrorMessage(response, 'Reve image generation failed'), { stage: 'image:reve' })
  }

  const bytes = new Uint8Array(await response.arrayBuffer())
  if (bytes.byteLength === 0) {
    throw InfraError('Reve image generation returned an empty image', { stage: 'image:reve' })
  }
