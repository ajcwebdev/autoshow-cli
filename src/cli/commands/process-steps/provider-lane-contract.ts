...
export const normalizeProviderLaneScopeLabel = (
  ...
  if (!/^[a-zA-Z0-9][a-zA-Z0-9._:-]{0,127}$/.test(normalized)) {
    throw new Error('Provider lane scope labels must be stable non-secret identifiers using letters, numbers, dot, underscore, colon, or hyphen.')
  }
  if (/^[a-f0-9]{64}$/i.test(normalized) || /^(?:sk|key|token|secret)[-_][a-z0-9_-]{12,}$/i.test(normalized)) {
    throw new Error('Provider lane scope labels must be non-secret names, not credentials or credential hashes.')
  }
  return normalized
}
