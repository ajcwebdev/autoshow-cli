...
const TTS_PROVIDERS = new Set(['kitten', 'elevenlabs', 'minimax', 'groq', 'grok', 'mistral', 'openai', 'gemini', 'deepgram', 'speechify', 'hume', 'cartesia', 'fish', 'inworld', 'deepinfra', 'replicate', 'fal'])
...
const assertAllowedKeys: (value: unknown, allowed: readonly string[], label: string) => asserts value is Record<string, unknown> = (value, allowed, label) => {
  if (typeof value !== 'object' || value === null || Array.isArray(value)) throw CLIUsageError(`${label} must be an object.`)
  const allowedSet = new Set(allowed)
  const unknown = Object.keys(value).filter(key => !allowedSet.has(key))
  if (unknown.length > 0) throw CLIUsageError(`${label} contains unsupported field(s): ${unknown.join(', ')}.`)
}

const assertSafeKey = (value: string, label: string): void => {
  if (!SAFE_KEY.test(value)) throw CLIUsageError(`${label} must be a safe lowercase opaque key.`)
}

const assertSubjectKey = (value: string, label: string): void => {
  if (!LOGICAL_SUBJECT_KEY.test(value)) throw CLIUsageError(`${label} must be a safe character key or explicit role:/voice: logical key.`)
}
...
const assertNonSecretReference = (value: string, label: string): void => {
  if (!value.trim() || /@|[\\/]|(?:^|\s)(?:name|email|contact)\s*[:=]/i.test(value) || value.includes('\0')) {
    throw CLIUsageError(`${label} must be an opaque non-contact reference.`)
  }
}
...
const validatePlannedCost = (cost: { amounts: Array<{ amount: number, currency: string }> }, label: string): void => {
  ...
  if (!Array.isArray(cost.amounts)) throw CLIUsageError(`${label} amounts must be an array.`)
  for (const amount of cost.amounts) {
    assertAllowedKeys(amount, ['amount', 'currency'], `${label} amount`)
    if (!Number.isFinite(amount.amount) || amount.amount < 0 || !amount.currency.trim()) throw CLIUsageError(`${label} contains an invalid currency amount.`)
  }
}
...
export const validateAuditActorRef = (actor: AuditActorRef): AuditActorRef => {
  ...
  if (!AUDIT_ACTOR_NAMESPACES.has(actor.namespace)) throw CLIUsageError('Audit actor namespace must be local-user, project-role, or automation.')
...
export const validateVoiceConsentRecord = (record: VoiceConsentRecord): VoiceConsentRecord => {
  ...
  if (record.schemaVersion !== 1) throw CLIUsageError('Voice consent record requires schemaVersion 1.')
  ...
  if (!['active', 'revoked', 'expired'].includes(record.status)) throw CLIUsageError('Voice consent status is unsupported.')
  ...
  if (!Array.isArray(record.grants)) throw CLIUsageError('Voice consent grants must be an array.')
  ...
  for (const grant of record.grants) {
    ...
    if (!CONSENT_ACTIONS.has(grant.action) || typeof grant.allowed !== 'boolean') throw CLIUsageError('Voice consent grant has an unsupported action or non-Boolean decision.')
    if (grant.expiresAt) {
      assertIsoDate(grant.expiresAt, `Consent ${grant.action} expiry`)
      if (Date.parse(grant.expiresAt) <= Date.parse(record.recordedAt)) {
        throw CLIUsageError(`Consent ${grant.action} expiry must follow recordedAt.`)
      }
    }
    if (grant.obligationRef) assertNonSecretReference(grant.obligationRef, `Consent ${grant.action} obligation`)
  }
  if (record.status === 'revoked') {
    if (!record.revokedAt || !record.revocationReason?.trim()) throw CLIUsageError('Revoked consent requires revocation time and reason.')
    assertIsoDate(record.revokedAt, 'Consent revokedAt')
  } else if (record.revokedAt !== undefined || record.revocationReason !== undefined) {
    throw CLIUsageError('Only revoked consent may contain revocation details.')
  }
  return record
}
...
export const assertVoiceConsentAllows = (
  ...
  if (!record) throw CLIUsageError(`Voice consent is required for ${action}; absent permission defaults to deny.`)
  validateVoiceConsentRecord(record)
  if (record.status !== 'active') throw CLIUsageError(`Voice consent is ${record.status}; ${action} is denied.`)
  const grant = record.grants.find(entry => entry.action === action)
  if (!grant?.allowed) throw CLIUsageError(`Voice consent does not permit ${action}.`)
  if (grant.expiresAt && Date.parse(grant.expiresAt) <= at.getTime()) {
    throw CLIUsageError(`Voice consent for ${action} has expired.`)
  }
}
...
const validateProvisioningState = (state: VoiceProvisioningState): void => {
              ...
              : state.state === 'expired' ? ['state', 'providerVoice']
                : state.state === 'deleted' ? ['state', 'providerVoice', 'deletedAt']
                  : state.state === 'failed' ? ['state', 'code', 'message', 'providerVoice']
                    : []
  if (allowed.length === 0) throw CLIUsageError('Voice provisioning state is unsupported.')
  ...
  if (state.state === 'ready' && !state.providerVoice) throw CLIUsageError('Ready provisioning requires a provider voice.')
  if (state.state === 'reconciliation-required' && (!state.attemptId.trim() || !state.reason.trim())) {
    throw CLIUsageError('Reconciliation-required provisioning needs attempt identity and reason.')
  }
  if (state.state === 'deleted') assertIsoDate(state.deletedAt, 'Voice provisioning deletedAt')
  if (state.state === 'failed' && (!state.code.trim() || !state.message.trim())) throw CLIUsageError('Failed provisioning requires a code and sanitized message.')
}
...
export const validateVoiceProvisioningAttempt = (
  ...
  if (attempt.schemaVersion !== 1) throw CLIUsageError('Voice provisioning attempt requires schemaVersion 1.')
  if (!PROVISIONING_OPERATIONS.has(attempt.operation)) throw CLIUsageError('Voice provisioning attempt has an unsupported operation.')
  ...
  if (attempt.reconciliation) {
    ...
    if (!['provider-operation', 'idempotency-lookup', 'provider-search', 'manual-inspection'].includes(attempt.reconciliation.strategy)) throw CLIUsageError('Voice provisioning reconciliation strategy is unsupported.')
    ...
    if (attempt.reconciliation.providerHandle !== undefined && !attempt.reconciliation.providerHandle.trim()) {
      throw CLIUsageError('Provisioning reconciliation provider handle cannot be blank.')
    }
  }
  if (!Number.isInteger(attempt.compareAndSwapVersion) || attempt.compareAndSwapVersion < 0) {
    throw CLIUsageError('Provisioning compare-and-swap version must be a non-negative integer.')
  }
  if (!Array.isArray(attempt.transitions) || !Array.isArray(attempt.issuedResources)) throw CLIUsageError('Provisioning transitions and issued resources must be arrays.')
  if (attempt.transitions.length === 0 || attempt.transitions[0]?.phase !== 'prepared') {
    throw CLIUsageError('Provisioning transitions must begin with prepared.')
  }
  ...
  for (const [index, transition] of attempt.transitions.entries()) {
    ...
    if (!['prepared', 'request-sent', 'response-received', 'ambiguous', 'reconciled', 'terminal'].includes(transition.phase)) throw CLIUsageError('Voice provisioning transition phase is unsupported.')
    if (transition.sequence !== index + 1) throw CLIUsageError('Provisioning transitions require contiguous one-based sequence numbers.')
  ...
  if (phases.filter(phase => phase === 'request-sent').length > 1) throw CLIUsageError('Provisioning attempt cannot send its create request more than once.')
  if (phases.includes('response-received') && !phases.includes('request-sent')) throw CLIUsageError('Provisioning response requires a prior request.')
  if (phases.includes('ambiguous') && !phases.includes('request-sent')) throw CLIUsageError('Ambiguous provisioning requires a prior request.')
  if (phases.includes('reconciled') && !phases.includes('ambiguous')) throw CLIUsageError('Provisioning reconciliation requires a prior ambiguous outcome.')
  if (phases.includes('terminal') && attempt.outcome === undefined) throw CLIUsageError('Terminal provisioning requires an outcome.')
  ...
  if (outcomeVoice && outcomeVoice.kind === 'remote-resource' && !attempt.issuedResources.some(resource =>
    ...
    throw CLIUsageError('Provisioning outcome resource must first be retained in issuedResources.')
  }
  return attempt
}
...
export const validateVoiceCandidate = (candidate: VoiceCandidate): VoiceCandidate => {
  ...
  if (candidate.schemaVersion !== 1) throw CLIUsageError('Voice candidate requires schemaVersion 1.')
  ...
  if (candidate.providerCandidateId !== undefined && !candidate.providerCandidateId.trim()) throw CLIUsageError('Voice candidate provider ID cannot be empty.')
  if (!TTS_PROVIDERS.has(candidate.provider)) throw CLIUsageError('Voice candidate has an unsupported provider.')
  ...
  if (candidate.sourceVoice) {
    ...
    if (candidate.sourceVoice.provider !== candidate.provider) throw CLIUsageError('Voice candidate source voice belongs to another provider.')
  }
  if (candidate.operation === 'remix') {
    if (!candidate.sourceVoice || !candidate.eligibilitySnapshotHash) throw CLIUsageError('Remix candidate requires source voice and eligibility snapshot identity.')
    assertSha256(candidate.eligibilitySnapshotHash, 'Remix eligibility snapshot hash')
  } else if (candidate.sourceVoice || candidate.eligibilitySnapshotHash) throw CLIUsageError('Only remix candidates may carry remix source eligibility.')
  assertIsoDate(candidate.createdAt, 'Candidate createdAt')
  if (!candidate.providerModel.trim()) throw CLIUsageError('Voice candidate requires a provider model.')
  if (!PROVISIONING_OPERATIONS.has(candidate.operation)) throw CLIUsageError('Voice candidate has an unsupported operation.')
  if (!Array.isArray(candidate.previewAssets)) throw CLIUsageError('Voice candidate preview assets must be an array.')
  ...
  if (candidate.expiryState === 'known') {
    if (!candidate.expiresAt) throw CLIUsageError('A voice candidate with known expiry requires expiresAt.')
    assertIsoDate(candidate.expiresAt, 'Candidate expiresAt')
    if (Date.parse(candidate.expiresAt) <= Date.parse(candidate.createdAt)) throw CLIUsageError('Voice candidate expiry must follow its creation time.')
  } else if (candidate.expiresAt !== undefined) {
    throw CLIUsageError('Voice candidate expiresAt is valid only when expiryState is known.')
  }
  if (candidate.materialization.state === 'materialized') {
    ...
    if (candidate.materialization.providerVoice.provider !== candidate.provider) throw CLIUsageError('Materialized candidate voice provider does not match its candidate.')
    ...
    if (!candidate.materialization.error.code.trim() || !candidate.materialization.error.message.trim()) throw CLIUsageError('Blocked candidate materialization requires a sanitized error.')
    ...
    throw CLIUsageError('Voice candidate has an unsupported materialization state.')
  }
  return candidate
}
...
export const validateVoiceAuditionManifest = (manifest: VoiceAuditionManifest): VoiceAuditionManifest => {
  ...
  if (manifest.schemaVersion !== 1) throw CLIUsageError('Voice audition manifest requires schemaVersion 1.')
  ...
  if (!TTS_PROVIDERS.has(manifest.provider)) throw CLIUsageError('Voice audition has an unsupported provider.')
  ...
  if (manifest.providerVoice.provider !== manifest.provider) throw CLIUsageError('Audition voice provider does not match its target provider.')
  if (manifest.settingsSchema !== manifest.synthesisSettings.settingsSchema) throw CLIUsageError('Audition settings schema does not match its settings payload.')
  assertIsoDate(manifest.createdAt, 'Audition createdAt')
  if (!Array.isArray(manifest.items) || !Array.isArray(manifest.warnings) || manifest.warnings.some(warning => typeof warning !== 'string')) throw CLIUsageError('Voice audition items and warnings must be arrays.')
  ...
  for (const category of AUDITION_REQUIRED_CATEGORIES) {
    if (!manifest.items.some(item => item.category === category)) throw CLIUsageError(`Canonical audition is missing required ${category} coverage.`)
  }
  for (const item of manifest.items) {
    ...
    if (!Array.isArray(item.takes) || !item.canonicalText.trim() || !item.providerText.trim() || item.takes.length === 0) throw CLIUsageError('Audition items require canonical/provider text and at least one take.')
    if (!['neutral', 'representative', 'emotional-delivery', 'pronunciation', 'comparison'].includes(item.category)) throw CLIUsageError('Voice audition item has an unsupported category.')
    ...
    if (!selected) throw CLIUsageError(`Audition ${item.itemId} selected take is missing.`)
    for (const take of item.takes) {
      ...
      if (take.sha256 !== take.protectedAudio.sha256) throw CLIUsageError('Audition take checksum must equal its protected asset checksum.')
      if (!Array.isArray(take.warnings) || take.warnings.some(warning => typeof warning !== 'string')) throw CLIUsageError('Voice audition take warnings must be strings.')
      validatePlannedCost(take.cost, 'Voice audition take cost')
      if (take.durationMs !== undefined && (!Number.isFinite(take.durationMs) || take.durationMs <= 0)) throw CLIUsageError('Audition take duration must be positive when present.')
    }
  }
  return manifest
}
...
export const validateVoiceRegistration = (registration: VoiceRegistration): VoiceRegistration => {
  ...
  if (!TTS_PROVIDERS.has(registration.provider)) throw CLIUsageError('Voice registration has an unsupported provider.')
  ...
  if (typeof registration.synthesisSettings.values !== 'object' || registration.synthesisSettings.values === null || Array.isArray(registration.synthesisSettings.values)) throw CLIUsageError('Voice registration synthesis settings values must be an object.')
  for (const [key, value] of Object.entries(registration.synthesisSettings.values)) {
    if (!key.trim() || !(value === null || ['string', 'number', 'boolean'].includes(typeof value) || (Array.isArray(value) && value.every(entry => typeof entry === 'string')))) throw CLIUsageError('Voice registration synthesis settings contain an unsupported value.')
  }
  if (typeof registration.sanitizedProviderMetadata !== 'object' || registration.sanitizedProviderMetadata === null || Array.isArray(registration.sanitizedProviderMetadata)) throw CLIUsageError('Voice registration provider metadata must be an object.')
...
export const validateVoiceRegistrationCatalog = (catalog: VoiceRegistrationCatalog): VoiceRegistrationCatalog => {
  ...
  if (catalog.schemaVersion !== 1) throw CLIUsageError('Voice registration catalog requires schemaVersion 1.')
  if (!Array.isArray(catalog.registrations)) throw CLIUsageError('Voice registration catalog registrations must be an array.')
  ...
  for (const registration of catalog.registrations) {
    if (registration.priorGenerationId) {
      ...
      if (!prior) throw CLIUsageError('Voice registration prior generation is missing from the append-preserving catalog.')
      if (prior.registrationId !== registration.registrationId) throw CLIUsageError('Voice registration prior generation belongs to a different registration chain.')
    }
  }
