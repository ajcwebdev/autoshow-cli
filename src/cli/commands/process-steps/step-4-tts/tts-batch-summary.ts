...
const simulateHostedProviderLane = (
  ...
  const selectJob = (): HostedEstimateJob | undefined => {
    const baseEligible = jobs
      .filter((job) => hasRemaining(job) && job.active === 0)
      .sort(compareHostedEstimateJobs)
    if (baseEligible.length > 0) {
      return baseEligible[0]
    }
    return jobs.filter(hasRemaining).sort(compareHostedEstimateJobs)[0]
  }
