...
export const runMultiSpeakerTts = async (
  ...
  const runSegment = async (i: number): Promise<string> => {
  ...
  if (target.service === 'kitten') {
    for (let i = 0; i < dialogue.turns.length; i++) {
      segmentPaths.push(await runSegment(i))
    }
  } else {
    const orderedSegmentPaths = await Promise.all(
      dialogue.turns.map((_, index) => runSegment(index))
    )
    segmentPaths.push(...orderedSegmentPaths)
  }
