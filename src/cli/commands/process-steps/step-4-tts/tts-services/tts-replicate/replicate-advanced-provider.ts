...
const capabilityRecords = [
  { scope: { provider: 'replicate', feature: 'turn-synthesis' as const }, maturity: 'stable' as const, channel: 'api' as const, adapterSupport: 'implemented' as const, requirements: [], constraints: { voiceKinds: ['provider-id' as const], supportedOutputFormats: ['wav'] }, documentationEvidence: evidence([DOCS.f5tts, DOCS.predictions]) },
  ...
  { scope: { provider: 'replicate', feature: 'instant-clone' as const }, maturity: 'not-applicable' as const, channel: 'unsupported' as const, adapterSupport: 'unsupported' as const, requirements: [], constraints: { requiresConsent: true, createsRemoteResource: false }, reason: unsupportedManagement, documentationEvidence: evidence([DOCS.f5tts]) },
