import type { LlamaIdentityMatchResult, LlamaServerIdentity, LlamaServerTarget } from '~/types'
...
export const resolveLlamaRequestModel = (identity: LlamaServerIdentity): string => {
  ...
  if (!requestModel) {
    throw InfraError(`llama-server is healthy but did not report a usable model id (${describeLlamaServerIdentity(identity)})`, { stage: 'write:llama' })
  }
  return requestModel
}
