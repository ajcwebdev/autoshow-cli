...
export const startDownloadProgressWatch = (downloadInfo: DownloadInfo): (() => void) => {
  ...
  const timer = setInterval(async () => {
    ...
    if (totalBytes !== null && totalBytes > 0) {
      ...
      l.write('info', `llama model download ${bar} ${percent.toFixed(1)}% (${formatBytes(currentBytes)}/${formatBytes(totalBytes)} @ ${formatBytes(bytesPerSec)}/s)`)
    } else {
      const spinner = spinnerFrames[spinnerIndex % spinnerFrames.length] || '-'
      spinnerIndex++
      // Visible at the default level: a multi-minute silent download is the
      // case this renderer exists for, and debug hid it from everyone.
      l.write('info', `llama model download ${spinner} ${formatBytes(currentBytes)} @ ${formatBytes(bytesPerSec)}/s`)
    }

    // This renderer already proves the llama task is alive, so the setup
    // heartbeat should leave it out of its aggregate line.
    noteSetupTaskActivity()
