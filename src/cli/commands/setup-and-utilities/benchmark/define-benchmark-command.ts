...
export const benchmarkCommand = defineCliCommand({
  name: 'benchmark',
  description: 'Benchmark STT transcription quality, score an existing TTS/text run, or judge an existing image/video run',
  parameters: [{ key: '[input]', description: 'Audio file path to benchmark, TTS run directory with --tts, write run directory with --text, image run directory with --image, or video run directory with --video' }],
  flags: {
    tts: {
    ...
    text: {
      description: 'Score an existing write run directory without calling LLM providers',
      type: Boolean,
      default: false
    },
    image: {
