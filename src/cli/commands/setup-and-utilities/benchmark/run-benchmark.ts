...
import { runTextBenchmark } from './run-text-benchmark'
...
const prepareSourceAudio = async (
  ...
  if (exitCode !== 0) {
    throw InfraError(`Failed to prepare source audio: ${stderr}`, { stage: 'benchmark:source-audio' })
  }
...
export const runBenchmark = async (
  ...
  if (selectedModes > 1) {
    throw CLIUsageError('Choose only one benchmark mode: --image, --text, --tts, or --video')
  }
  ...
  if (flags.text === true) {
    await runTextBenchmark(input, flags)
    return
  }
  ...
  const pinnedRunDir = claimPinnedRunDir('benchmark')
  const outputDir = pinnedRunDir
    ? resolve(pinnedRunDir)
  ...
  if (refResult.error) {
    throw InfraError(`Reference transcription failed: ${refResult.error}`, { stage: 'benchmark:reference-stt' })
  }
