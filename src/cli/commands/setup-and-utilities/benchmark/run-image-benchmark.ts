...
const requireScore = (object: JsonObject, key: keyof ImageCriterionScores): number => {
  ...
  if (value === undefined || value < 1 || value > 10) {
    throw ValidationError(`OpenAI image judge response field ${key} must be a number from 1 through 10.`, { stage: 'benchmark:image-judge' })
  }
  return value
}
...
const parseImageJudgeResponse = (rawText: string, fileName: string): ImageEvaluation => {
  ...
  if (!summary) {
    throw ValidationError('OpenAI image judge response field summary must be a non-empty string.', { stage: 'benchmark:image-judge' })
  }
...
export const runImageBenchmark = async (
  ...
  if (!input) {
    throw CLIUsageError('Image run directory is required. Usage: bun autoshow benchmark <image-run-dir> --image')
  }
