...
export const resolveUvAssetName = (
  ...
  throw InternalError(`Unsupported platform for managed uv setup: ${platform}/${arch}`, { stage: 'setup:uv' })
}
...
const findExtractedBinary = async (root: string, name: 'uv' | 'uvx'): Promise<string> => {
  ...
  if (!found) {
    throw InfraError(`Managed uv archive did not contain ${name}`, { stage: 'setup:uv' })
  }
  return found
}
