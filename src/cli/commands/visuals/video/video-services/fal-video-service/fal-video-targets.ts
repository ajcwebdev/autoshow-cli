...
export const collectFalVideoTargets = (options: VideoGenOptions, mode: VideoMode): VideoTarget[] => {
  ...
  return models.flatMap((rawModel) => {
    ...
    const effective = {
      duration: normalizeFalVideoDuration(model, options.videoDuration),
      resolution: normalizeFalVideoResolution(model, options.videoResolution),
      aspectRatio: normalizeFalVideoAspectRatio(model, options.videoAspectRatio, mode)
    }
    ...
    const request = { model, mode, duration: options.videoDuration, resolution: options.videoResolution, aspectRatio: options.videoAspectRatio, inputImage: options.videoInputImage, lastFrame: options.videoLastFrame, referenceImages: options.videoReferenceImages, referenceVideos: options.videoReferenceVideos, referenceAudios: options.videoReferenceAudios, generateAudio: options.videoGenerateAudio }
    return [{
      service: 'fal',
      model,
      requestSettings: { ...request, effective },
      run: async (prompt, outputDir) => {
        if (prompt === undefined) throw UsageError('fal.ai video prompt cannot be empty.')
        return await runFalVideoGen(prompt, outputDir, request)
      }
    }]
  })
}
