...
export const runGeminiVideoGen = async (
  ...
  return await runVideoGeneration({
    ...
    prepare: async (context) => {
      ...
      // Reference uploads stage files under the run directory, so it has to exist before they run.
      ...
      if (mode === 'image-to-video' && options.inputImage) {
        ...
        for (const video of options.referenceVideos ?? []) {
          const uri = await uploadOmniVideo(context.apiKey, video, outputDir, options.abortSignal, {
        ...
        const uri = await uploadOmniVideo(context.apiKey, options.inputVideo, outputDir, options.abortSignal, {
    ...
    execute: async (context, prepared) => {
      ...
      }, async (signal) => await geminiCreateInteraction(context.apiKey, prepared.body, signal), (error) => classifyFetchRetry(error, 'runtime_http_create_conservative'))
      ...
      if (parsed.status !== 'completed') {
        ...
        parsed = parseInteractionVideo(await pollUntil({
          ...
          pollFn: async () => await withRetry({
            ...
            context.logStatus('in_progress')
            return await geminiGetInteraction(context.apiKey, interactionId, signal)
      ...
      await writeOmniVideo(context.apiKey, parsed, outputPath, options.abortSignal)

      return {
        artifactPaths: [outputPath],
        completionDetail: `Actual billed cost was not returned by the API; estimate ${prepared.estimate.totalCost.toFixed(3)}¢`,
        metadata: {
          videoDuration: options.durationSeconds !== undefined ? prepared.billedDuration : undefined,
          requestMode: mode,
          videoResolution: prepared.normalizedResolution,
          videoAspectRatio: prepared.normalizedAspectRatio,
