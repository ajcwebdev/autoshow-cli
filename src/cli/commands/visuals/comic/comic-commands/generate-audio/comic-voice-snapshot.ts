...
import { resolveCharacterVoiceRegistryPaths } from '../../../../audio/voice/character-voice-registry'
...
export const resolveComicVoiceSnapshot = async (input: {
  ...
  if (registryPresence.some(Boolean) && !registryPresence.every(Boolean)) throw UsageError('Character voice registry is incomplete; briefs, registrations, and current selections must be present together.')
  ...
  if (!snapshot) throw UsageError('Comic audio requires a retained voice snapshot or a complete character voice registry.')
