
export const REVISION_IMAGE_MODEL = 'gpt-image-2'

export const REVISION_COMPARISON_PROVIDER = 'openai'

export const REVISION_COMPARISON_MODEL = 'gpt-5.6-sol'

export const REVISION_COMPARISON_PASSES = 2

export const REVISION_ESTIMATED_INPUT_TOKENS_PER_COMPARISON = 6500

export const REVISION_ESTIMATED_OUTPUT_TOKENS_PER_COMPARISON = 1200

export const IMPORTANCE_VALUES = ['critical', 'high', 'medium', 'low', 'not-meaningful'] as const

export const DEFECT_CATEGORY_VALUES = ['cast', 'dialogue-speaker', 'identity-costume', 'location-topology', 'set-continuity', 'staging-framing', 'source-prop', 'mixed', 'false-positive'] as const
