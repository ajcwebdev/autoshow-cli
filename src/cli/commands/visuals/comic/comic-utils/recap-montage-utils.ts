...
import type { RecapMontageExpansion, RecapMontageSceneEntry, ScenePromptData, StructuredScriptData, StructuredScriptSourceSegment } from '~/types'
  ...
  const entries = await (async () => {
    try {
      ...
      throw InfraError(
        ...
        { stage: 'comic:recap-montage', ...(error instanceof Error ? { cause: error } : {}) }
      )
    }
  })()
...
const buildPriorSceneEntry = async (scriptPath: string): Promise<RecapMontageSceneEntry> => {
  ...
  return {
    ...
    characterKeys: structuredScript.characterKeys,
...
const formatCharacters = (characterKeys: string[]): string => {
  return characterKeys.length > 0 ? characterKeys.join(', ') : 'None named'
}
...
const formatSceneEntry = (scene: RecapMontageSceneEntry, index: number): string => {
  ...
  return [
    ...
    `   - Character keys: ${formatCharacters(scene.characterKeys)}`,
