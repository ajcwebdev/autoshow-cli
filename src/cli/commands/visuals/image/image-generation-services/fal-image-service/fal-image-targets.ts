...
import { FAL_IMAGE_COUNT_RANGE, runFalImageGen } from './run-fal-image-gen'
...
export const collectFalImageTargets = (options: ImageGenOptions): ImageTarget[] => {
  ...
  return models.map((rawModel) => {
    ...
    assertNoUnsupportedFlags(options, [
      ...
      provider: 'fal.ai',
      model,
      hint: 'fal.ai image support varies by model: common controls are --format and --count; HiDream and Qwen use --size rather than --aspect-ratio.'
    })
    ...
    const maxInputs = model === 'alibaba/qwen-image-3' ? 3 : 9
    ...
    const request = { model, inputs: options.imageInputs, imageSize: options.imageSize, aspectRatio: options.imageAspectRatio, count: options.imageCount, outputFormat: options.imageFormat }
    return {
      service: 'fal' as const,
      model,
      requestSettings: request,
      run: async (prompt, outputDir) => await runFalImageGen(prompt, outputDir, request)
    }
  })
}
