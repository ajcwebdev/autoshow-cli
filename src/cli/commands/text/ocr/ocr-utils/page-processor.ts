...
import { extractPageText, renderPageToImage } from '~/cli/commands/sources/download/document/mutool-utils'
    ...
    const rendered = await getCachedRenderedPageImage(
      ...
      async (outputPath) => {
        ...
        if (renderResult.exitCode !== 0) {
          throw InfraError(renderResult.stderr || `Failed rendering page ${page}`, { stage: 'ocr:render' })
        }
      }
    )
