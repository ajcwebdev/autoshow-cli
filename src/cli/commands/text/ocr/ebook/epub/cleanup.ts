import type { ExtractionState, HtmlRewriterConstructor, HtmlRewriterElement } from '~/types'
...
const findHtmlTagEnd = (html: string, start: number): number => {
  let quote: '"' | "'" | undefined
...
const findSelfClosingSlash = (tag: string): number | undefined => {
  for (let index = tag.length - 2; index >= 0; index--) {
...
const isHtmlVoidTag = (tagName: string): boolean => {
  const normalized = tagName.toLowerCase()
  const localName = normalized.includes(':') ? normalized.split(':').at(-1) ?? normalized : normalized
  return HTML_VOID_TAGS.has(normalized) || HTML_VOID_TAGS.has(localName)
}

const normalizeSelfClosingXhtmlTags = (html: string): string => {
  let result = ''
  let lastCopied = 0
  let searchFrom = 0
  let changed = false
  ...
  while (searchFrom < html.length) {
    ...
    if (html.startsWith('<![CDATA[', tagStart)) {
      ...
      searchFrom = cdataEnd === -1 ? html.length : cdataEnd + 3
      continue
    }
    if (html.startsWith('<?', tagStart)) {
      const processingEnd = html.indexOf('?>', tagStart + 2)
      searchFrom = processingEnd === -1 ? html.length : processingEnd + 2
      continue
    }
    if (html.startsWith('<!', tagStart)) {
      const declarationEnd = findHtmlTagEnd(html, tagStart)
      searchFrom = declarationEnd === -1 ? html.length : declarationEnd + 1
      continue
    }
    ...
    const slashIndex = findSelfClosingSlash(tag)
    if (slashIndex === undefined) {
      continue
    }

    const tagName = tag.match(/^<\s*([A-Za-z][A-Za-z0-9:._-]*)\b/)?.[1]
    if (!tagName || isHtmlVoidTag(tagName)) {
      continue
    }

    const startTag = `${tag.slice(0, slashIndex).replace(/\s+$/, '')}>`
    result += html.slice(lastCopied, tagStart)
    result += `${startTag}</${tagName}>`
    lastCopied = tagEnd + 1
    changed = true
  }

  return changed ? result + html.slice(lastCopied) : html
}
...
const getHtmlRewriter = (): HtmlRewriterConstructor => {
  ...
  if (typeof HTMLRewriterCtor !== 'function') {
    throw InfraError('Bun HTMLRewriter is required for EPUB HTML cleanup.', { stage: 'ocr:epub' })
  }
...
export const cleanEpubHtmlToText = async (html: string): Promise<string> => {
  const normalizedHtml = normalizeSelfClosingXhtmlTags(html)
  ...
  await rewriter.transform(new Response(normalizedHtml)).text()
