import { showPdfObject, showPdfOutline } from '~/cli/commands/sources/download/document/mutool-utils'
import type { PageResult, PdfChapterBuildResult, PdfChapterDetectionSummary, PdfChapterMode, PdfOutlineEntry, PdfPageLabelEntry, ResolvedPdfChapter, TextArtifactFile } from '~/types'
import * as l from '~/utils/app-logger/app-logger'
import { buildChapterArtifactRelativePaths } from '../../chapter-artifact-filenames'
import { splitWithHardLimit } from '../../ebook/epub/export'
import {
...
export const buildPdfChapterFiles = (
  ...
  const fileParts: Array<{
    chapter: ResolvedPdfChapter
    ordinal: number
    text: string
    partIndex?: number
  }> = []
  let emittedChapterCount = 0
  ...
  for (let index = 0; index < chapters.length; index++) {
    ...
    const ordinal = emittedChapterCount + 1
    emittedChapterCount += 1
  ...
  const relativePaths = buildChapterArtifactRelativePaths(fileParts.map((filePart) => ({
    ...
    sourceLocator: filePart.chapter.pdfStartPage,
    slug: buildChapterSlug(filePart.chapter.title, filePart.chapter.pdfStartPage),
...
export const buildPdfChapterArtifacts = async (input: {
  ...
  chapters = dedupeResolvedChapters(chapters)
    .map((chapter) => ({
      ...chapter,
      title: cleanDetectedChapterTitle(chapter.title)
    }))
    .filter((chapter) => chapter.pdfStartPage >= 1 && chapter.pdfStartPage <= totalPages)
