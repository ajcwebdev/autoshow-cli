...
export const runGrokTts = async (
  ...
  const apiKey = requireTtsCredential('grok')
  ...
  const chunks = resolveTtsDispatchChunks({ provider: 'grok', model: options.model, text: text, chunking: options.chunking }, options.requestEvidence)
