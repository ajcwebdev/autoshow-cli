...
const capabilityRecords = [
  ...
  { scope: { provider: 'speechify', feature: 'word-timing' as const }, maturity: 'stable' as const, channel: 'api' as const, reason: 'Speech marks are not retained by the current audio-only adapter.', adapterSupport: 'planned' as const, requirements: [], constraints: { providerTimeUnit: 'milliseconds' }, documentationEvidence: evidence([DOCS.synthesis]) },
