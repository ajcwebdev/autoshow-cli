...
import { TTS_DELIVERY_SEAM_BOUNDARIES } from '~/types/tts-workflow/tts-types'
...
export const validateGeminiJobRun = (run: GeminiProviderJobRun): GeminiProviderJobRun => {
  ...
  for (const slot of run.slots) {
    ...
    if (slot.boundaryAfter !== undefined && !(TTS_DELIVERY_SEAM_BOUNDARIES as readonly string[]).includes(slot.boundaryAfter)) fail()
