import type { PlannedTtsChunk, TtsChunkBoundary } from '~/types'
...
// Frozen algorithms used only to reproduce the text of retained paid plans.
export const replaySavedTtsChunks = (text: string, maxChars: number, version: 'legacy-v0' | 'smart-v1', adjustBoundary?: (text: string, index: number, limit: number) => number): PlannedTtsChunk[] =>
  version === 'smart-v1' ? planSmartChunks(text, maxChars, adjustBoundary) : planLegacyChunks(text, maxChars, adjustBoundary)
