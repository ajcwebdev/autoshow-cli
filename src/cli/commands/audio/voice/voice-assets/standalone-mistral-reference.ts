...
import { preflightTtsTargetSelection } from '../../tts/tts-targets/tts-target-collect'
import { MISTRAL_CLI_REFERENCE_AUTHORIZATION } from './mistral-request-reference-policy'
import { isMultiSpeakerRequested, parseSpeakerVoiceMappings } from '../../tts/dialogue-normalizer'
import { assertProtectedStoreOutputDisjoint } from './protected-output-boundary'
import { getOutputRootAbsolute } from '~/cli/commands/command-shared/output-root'
...
export const planStandaloneMistralReference = async <T extends TtsOptions>(
  ...
  if (referenceInput.authorizationRef !== MISTRAL_CLI_REFERENCE_AUTHORIZATION) {
    throw UsageError(
      ...
      { hints: ['Pass --mistral-tts-ref-audio explicitly to standalone `tts`, or create/import a voice with the shared `voice` command and synthesize with --mistral-tts-voice.'] }
    )
  }
  if (referenceInput.speakerKey) {
    throw UsageError(
      'Per-speaker Mistral reference inputs are not available in the Phase 0 standalone path.',
      { hints: ['Create or import each voice with the shared `voice` command, then use its existing voice ID in dialogue mappings.'] }
    )
  }
  if (isMultiSpeakerRequested(options)) {
    throw UsageError(
      ...
      { hints: ['Create or import each voice with the shared `voice` command, then use its existing voice ID in SPEAKER=VOICE mappings.'] }
    )
  }
