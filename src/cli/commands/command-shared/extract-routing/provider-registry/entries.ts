import type { Step2Command, Step2ModelProviderRegistryEntry, Step2ProviderConfigPathEntry, Step2ProviderRegistryEntry, Step2ShortcutFlag } from '~/types'
import { STEP2_OCR_PROVIDER_REGISTRY } from './ocr-providers'
import { STEP2_STT_PROVIDER_REGISTRY } from './stt-providers'
import { STEP2_URL_PROVIDER_REGISTRY } from './url-providers'
...
for (const entry of STEP2_PROVIDER_REGISTRY) {
  if (!STEP2_PROVIDER_ENTRY_BY_FLAG.has(entry.flagName)) {
...
export const getStep2ProviderConfigPathEntries = (): Step2ProviderConfigPathEntry[] =>
  [...new Map(STEP2_PROVIDER_REGISTRY.map((entry) => [
    entry.flagName,
    {
