...
export const normalizeWriteStepSelectorFlags = (
  ...
  applyFlagOccurrenceNormalization(flags, explicitFlags, flagOccurrences, (occurrence) => {
    const providerTargets = writeSelectorTargetsByFlag[occurrence.name as keyof typeof writeSelectorTargetsByFlag]
    if (providerTargets) {
    ...
    const targetsByStep: Partial<Record<keyof typeof writeAllProvidersTargets, string>> = flagName === 'all-providers'
