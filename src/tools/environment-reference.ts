...
Runtime controls include \`AUTOSHOW_PROJECT_ROOT\`, \`AUTOSHOW_SOURCE_IDENTITY_ROOT\`, \`AUTOSHOW_SOURCE_IDENTITY_ALIAS\`, \`AUTOSHOW_REQUIRED_IMAGE_MODEL\`, \`AUTOSHOW_DISABLE_HTTP_KEEPALIVE\`, and \`AUTOSHOW_HTTP_PAYLOAD_MAX_BYTES\`. The last is a positive whole number of bytes that replaces every HTTP payload ceiling: successful response bodies are read whole, and a body over the ceiling is rejected instead of truncated. The built-in ceilings are 16 MiB for control-plane JSON, 512 MiB for provider results (including media inlined as base64 or hex) and 2 GiB for in-memory downloads; downloads streamed to disk have no ceiling. A malformed value fails at startup, before any provider request. Internal setup handoffs use \`AUTOSHOW_SETUP_NO_ORPHANS_CHILD\` and \`AUTOSHOW_NETWORK_CHECK_CHILD_TIMEOUT_MS\`. Terminal color uses \`NO_COLOR\` and \`FORCE_COLOR\`; flags override these controls. These are separate from provider credentials.
