...
- Active fixtures are in `input/examples/tts/controls/emotion/` and `input/examples/tts/controls/speed-pauses/`; `input/examples/tts/controls/benchmark-plan.json` tracks their shared spending estimate. Use `bun src/tools/tts-controls-benchmark.ts --suite all --price` to review both. Save the suites separately under `docs/benchmarks/tts/`. The shared ledger’s `outputBase` is `docs/benchmarks/tts`; the independent benchmarks use `<YYYY-MM-DD>_05-tts-emotion/` and `<YYYY-MM-DD>_06-tts-speed-pauses/` beneath it, dated when the benchmark first runs and pinned in the shared ledger for subsequent reuse, within the existing TTS dashboard tab. Keep spending cumulative across both benchmarks. Preserve earlier run directories. Exclude all controls fixtures from all-provider directory batches.
