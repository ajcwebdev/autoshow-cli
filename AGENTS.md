...
- Active fixtures are in `input/examples/tts/controls/emotion/` and `input/examples/tts/controls/speed-pauses/`; `input/examples/tts/controls/benchmark-plan.json` tracks their shared spending estimate. Use `bun src/tools/tts-controls-benchmark.ts --suite all --price` to review both. Save the suites separately under `docs/benchmarks/tts/`. The shared ledger’s `outputBase` selects the active run; the detailed-instructions revision uses `docs/benchmarks/tts/2026-09-12_detailed-instructions/`. Preserve earlier run directories. The earlier mixed controls run and `05-tts-controls.txt` are historical. Exclude all controls fixtures from all-provider directory batches.
